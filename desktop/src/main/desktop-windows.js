...
function createDesktopWindows(app, options = {}) {
  ...
  function maybeShowCloseToTrayHint() {
    ...
    if (process.platform === "win32" && typeof tray.displayBalloon === "function") {
      tray.displayBalloon({
        ...
        content: "Window hidden to system tray, local. API Still usable. Right-click tray icon to exit.",
      });
    }
  ...
  function createWindow() {
    ...
    mainWindow.webContents.on("render-process-gone", (_event, details) => {
      ...
      dialog.showErrorBox("RetainPDF render process exited abnormally", detail);
    });

    mainWindow.webContents.once("did-finish-load", () => {
      logDesktop("[desktop] frontend loaded; showing main window");
      updateSplashProgress(100, "Ready", "Entering main screen.");
      mainWindow.show();
    ...
    mainWindow.webContents.setWindowOpenHandler(({ url }) => {
      ...
      // In-app pages: open in main window, prohibited shell.openExternalOtherwise like「Browser jump」）。
      // Neither. silent denyOtherwise compare reading entry「No response on click」。
      try {
...
// Ensure navigation within main window. "Side-by-side reading" wait for entry point availability.
          if (mainWindow && !mainWindow.isDestroyed()) {
