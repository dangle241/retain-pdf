...
async function preparePythonRuntime(pythonRuntime, options = {}) {
  ...
  if (!startupProbe.ok && selectedRuntime.bundledHome) {
    ...
    updateSplashProgress(options, 26, "Checking Python runtime", "Bundled Python not available, falling back to system Python");
  ...
  if (!dependencyProbe.ok) {
    if (options.isPackaged && selectedRuntime.bundledHome && dependencyProbe.timedOut) {
      ...
      updateSplashProgress(options, 26, "Checking Python runtime", "Bundled Python slow to start, continuing to start local services");
