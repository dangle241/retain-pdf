...
def _clone_form_xobject(pdf: pikepdf.Pdf, xobject: pikepdf.Object) -> pikepdf.Object:
    ...
    for key, value in xobject.items():
        if key in {Name("/Length"), Name("/Filter"), Name("/DecodeParms")}:
            continue
        # Keys whose value is PDF null (e.g. `/StampId null` on a publisher
        # stamp form) are, by spec, equivalent to the key not existing at
        # all. pikepdf also refuses to set a dict key to None, so we just
        # skip these entries.
        if value is None:
            continue
