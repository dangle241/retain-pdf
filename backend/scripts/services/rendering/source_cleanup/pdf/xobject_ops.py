...
def _clone_form_xobject(pdf: pikepdf.Pdf, xobject: pikepdf.Object) -> pikepdf.Object:
    ...
    for key, value in xobject.items():
        if key in {Name("/Length"), Name("/Filter"), Name("/DecodeParms")}:
            continue
# Value is a PDF null key (like publisher seal form /StampId null) according to
        # Equivalent to key absence per spec;pikepdf Also reject setting dict keys to None,Skip directly.
        if value is None:
            continue
