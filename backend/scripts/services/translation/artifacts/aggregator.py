    ...
    def configure_adaptive_concurrency(
        ...
        *,
        ...
        with self._adaptive_condition:
            # warmup:Allow only 1 Request,let provider Write prefix cache to public.
            # System prompt,After first request completes, resume full concurrency.——Subsequent requests hit cache.,
            # Avoid thundering herd on cold start.
        ...
        *,
        ...
        with self._adaptive_condition:
            ...
            if self._warmup_pending:
                # Restore full concurrency regardless of first request success or failure.:Preheat is best-effort.,On failure
                # Avoid pinning entire execution to serial.
        ...
        *,
        ...
        if not success and timeout_like:
            ...
            if high_capacity_provider:
                # Isolated timeout tolerance without throttling.;Continuous failure accumulation indicates network/provider edge
                # Degrading(Connection timeout storm confirmed in live testing. limit Nailed it. 100 Exacerbates thundering herd.),
                # Per accumulation 5 Decelerate gently one gear. Success resets counter.
