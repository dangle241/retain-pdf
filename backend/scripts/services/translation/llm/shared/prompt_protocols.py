...
# Chuỗi prompt tiếng Trung cũ: giữ nguyên để vẫn khớp và gỡ bỏ được
# các prompt template phiên bản cũ.
...
def _append_math_delimiter_damage_hint(lines: list[str], item: TranslationItemContext) -> None:
    # Khi $ trong văn bản gốc không cân bằng, đưa thẳng cho model chắc chắn sinh ra bản dịch
    # không cân bằng, kích hoạt cả chuỗi kiểm tra/sửa lỗi (đo thực tế: một mục tốn ~10 lần gọi
    # LLM). Vì vậy nhắc model sửa theo ngữ nghĩa trước.
...
def _append_mitex_rewrite_hint(lines: list[str], item: TranslationItemContext) -> None:
    # Gợi ý theo nhu cầu, dựa trên dữ liệu: chỉ khi văn bản gốc thật sự chứa lệnh mà
    # bộ render không hỗ trợ thì mới báo quy tắc thay thế cho model, để model thay thế
    # ở tầng ngữ nghĩa (với công thức phức tạp, viết lại bằng regex không đáng tin cậy);
    # việc viết lại bằng regex lúc render vẫn giữ lại làm phương án dự phòng.
...
def _append_scoped_terms_guidance(lines: list[str], item: TranslationItemContext) -> None:
    # Hướng dẫn thuật ngữ khớp theo từng mục được đặt trong message user: nếu đặt ở system
    # thì mỗi request sẽ có tiền tố khác nhau, làm mất cache tiền tố của provider.
