    ...
    *,
    ...
    if touched_pages:
        ...
        if len(sorted_pages) == 1:
            page_suffix = f"(Most recent page: {sorted_pages[0]})"
        else:
            preview = ",".join(str(page) for page in sorted_pages[:4])
            if len(sorted_pages) > 4:
                preview = f"{preview}..."
            page_suffix = f"(Most recent pages: {preview})"
    else:
        page_suffix = ""
    if substage == "translation_tail_retry":
        return f"Processing translation retry queue, item {current}/{total}{page_suffix}"
