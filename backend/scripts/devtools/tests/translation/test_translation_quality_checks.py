...
def test_quality_checks_collect_glossary_issues() -> None:
    ...
    report = review_translation_item(
        ...
        {
            "decision": "translate",
"translated_text": "This loop is initialized by orbitals, then iterated.",
        },
        glossary_entries=[
...
def test_quality_flags_abnormally_short_translation() -> None:
    ...
    short_zh = (
        "Corresponding diphenylamine group para position underwent tandem electrophilic substitution.C–HBorylation, possibly due to its electron-donating nature and steric bulk."
        "These processes are robust and scalable, by6.0 gSynthesis of target compound at scale verified."
    )
...
def test_quality_allows_normal_length_chinese_translation() -> None:
    ...
    translated = (
        "Commercial2-bromo-1,3-Difluorobenzene via lithium.-One of the most common starting materials for halogen exchange in one-pot borylation."
        "The first example of this type of synthesis was reported concurrently with the first directed ortho-lithiation method."
    )
...
def test_long_english_residue_span_skips_data_dense_nmr_segments() -> None:
    ...
    # NMR Spectral data is digit-dense,Legally retained data, not untranslated prose.
    nmr = (
    ...
    # Still need to catch genuine English prose remnants.
...
def test_agent_repair_skips_items_already_repaired_in_flight() -> None:
    ...
    fresh = {"item_id": "p001-b001", "translated_text": "Normal translation", "translation_diagnostics": {}}
    assert not _already_repaired_in_flight(fresh)


def test_fast_agent_repair_only_runs_with_blocking_items() -> None:
    # Skip warning candidates by length when task is clean.
