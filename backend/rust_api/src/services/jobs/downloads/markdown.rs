...
// Matches `![alt](images/...)`; paths can contain spaces; optional "title"/'title',
// angle brackets, and `./` prefix.
// The path uses a greedy `[^)>\n]+` match; the title must be quoted to avoid
// mistaking a space in `chart a.png` for a title separator.
const MARKDOWN_IMAGE_LINK_RE: &str =
    r#"!\[([^\]]*)\]\(\s*<?((?:\./)?images/[^)>\n]+)>?(?:[ \t]+(?:"[^"]*"|'[^']*'))?\s*\)"#;
// HTML: handle double-quoted attributes (the `regex` crate does not support backrefs).
...
fn markdown_images_view(
        ...
        .filter(|entry| entry.file_type().is_file())
    {
        ...
        // Path matches the markdown original reference: `images/<rel>`.
        // Note: when building `images_base_url`, the frontend must strip the
        // `images/` prefix, see `normalize_markdown_image_rel`.
...
/// Rewrite relative image references in markdown/html to direct
/// absolute API URLs. Also fix the frontend's potential "double images"
/// prefix bug (`images_base + images/...`): the absolute URL points
/// directly to `/markdown/images/<rel>`, bypassing base concatenation.
...
/// Normalize a markdown image relative path to a path relative to the
/// images directory (stripping the `images/` prefix).
fn normalize_markdown_image_rel(raw: &str) -> String {
    let mut path = raw.trim().trim_matches(|c| c == '<' || c == '>').to_string();
    // Strip optional title: only truncate if `path "title"` or `path 'title'`
    // is matched. Filenames can contain spaces (e.g. `chart a.png`), so we
    // cannot truncate at the first whitespace.
    ...
    // Strip one or more `images/` prefixes to avoid `images/images/...`
    // duplication.
