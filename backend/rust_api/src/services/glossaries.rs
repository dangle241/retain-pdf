...
fn normalize_glossary_level(value: &str) -> String {
    match sanitize_csv_cell(value).to_ascii_lowercase().as_str() {
        ...
        | "no-translate" | "không dịch" | "giữ nguyên" | "giữ nguyên bản gốc" => "preserve".to_string(),
        "canonical" | "fixed" | "fixed_translation" | "fixed-translation" | "required"
        | "bắt buộc dịch" | "dịch cố định" | "dịch chuyên ngành" | "dịch chuẩn" => {
            "canonical".to_string()
        }
...
fn normalize_glossary_match_mode(value: &str) -> String {
    match sanitize_csv_cell(value).to_ascii_lowercase().as_str() {
        ...
        | "không phân biệt hoa thường" | "bỏ qua hoa thường" => "case_insensitive".to_string(),
        _ => "exact".to_string(),
    }
}
...
fn detect_csv_header(row: &csv::StringRecord) -> Option<GlossaryCsvHeader> {
    ...
    for (index, value) in row.iter().enumerate() {
        ...
        match normalized.as_str() {
            "source" | "src" | "term" | "original" | "từ gốc" | "văn bản gốc" | "thuật ngữ" => {
                source_idx = Some(index)
            }
            "target" | "dst" | "translation" | "translated" | "bản dịch" | "dịch" | "dịch đích" => {
                target_idx = Some(index)
            }
            "note" | "notes" | "comment" | "comments" | "ghi chú" | "mô tả" => {
                note_idx = Some(index)
            }
            "level" | "glossary_level" | "mode" | "action" | "loại" | "chế độ" | "hành động" => {
                level_idx = Some(index)
            }
            "match" | "match_mode" | "match-mode" | "khớp" | "chế độ khớp" => {
                match_mode_idx = Some(index)
            }
            "context" | "ngữ cảnh" | "bối cảnh" => context_idx = Some(index),
            _ => {}
        }
    }
...
mod tests {
    ...
    fn normalize_glossary_entries_dedupes_and_trims() {
        let entries = normalize_glossary_entries(&[
            GlossaryEntryInput {
                ...
                target: " axit deoxyribonucleic ".to_string(),
                note: String::new(),
    ...
    fn preserve_glossary_entry_can_omit_target() {
        let entries = normalize_glossary_entries(&[GlossaryEntryInput {
            ...
            target: String::new(),
            note: String::new(),
            level: "không dịch".to_string(),
            match_mode: "bỏ qua hoa thường".to_string(),
            context: String::new(),
        }])
    ...
    fn parse_glossary_csv_supports_chinese_table_headers() {
        let entries = parse_glossary_csv_text("từ gốc,bản dịch,loại,chế độ khớp,ghi chú\nKohn-Sham,,giữ nguyên,bỏ qua hoa thường,method name\nDFT,density functional theory,dịch chuyên ngành,exact,expanded form\n")
            .expect("parse csv");
    ...
    fn glossary_csv_export_escapes_cells() {
        let record = GlossaryRecord {
            ...
            entries: vec![GlossaryEntryInput {
                ...
                target: "mật độ trạng thái".to_string(),
        ...
        assert!(export.csv_text.contains("\"mật độ trạng thái\""));
    }

    #[test]
    fn merge_glossary_entries_prefers_overlay() {
        let merged = merge_glossary_entries(
            &[entry("DNA", "axit deoxyribonucleic"), entry("abstract", "tóm tắt")],
            &[entry("DNA", "DNA"), entry("band gap", "khoảng cách vùng")],
        );
    ...
    fn resolve_task_glossary_request_merges_resource_and_inline_entries() {
        ...
        let glossary = create_glossary(
            ...
            &GlossaryUpsertInput {
                ...
                target_lang: "zh-CN".to_string(),
                enabled: true,
                entries: vec![entry("DNA", "axit deoxyribonucleic"), entry("abstract", "tóm tắt")],
            },
        )
        .expect("create glossary");
        let mut input = CreateJobInput::default();
        input.translation.glossary_id = glossary.glossary_id.clone();
        input.translation.glossary_entries = vec![entry("DNA", "DNA"), entry("band gap", "khoảng cách vùng")];
    ...
    fn glossary_crud_round_trip() {
        ...
        let created = create_glossary(
            ...
            &GlossaryUpsertInput {
                ...
                target_lang: "zh-CN".to_string(),
                enabled: true,
                entries: vec![entry("band gap", "khoảng cách vùng")],
            },
        )
        .expect("create glossary");
        let loaded =
        ...
        let updated = update_glossary(
            ...
            &GlossaryUpsertInput {
                ...
                target_lang: "zh-CN".to_string(),
                enabled: false,
                entries: vec![entry("band gap", "khoảng cách vùng"), entry("exciton", "exciton")],
            },
        )
    ...
    fn resolve_task_glossary_request_rejects_merged_entries_over_limit() {
        ...
        let resource_entries = (0..MAX_GLOSSARY_ENTRIES)
            .map(|index| entry(&format!("term-{index}"), &format!("từ-{index}")))
            .collect();
        let glossary = create_glossary(
            state.db.as_ref(),
            &GlossaryUpsertInput {
        ...
        input.translation.glossary_entries = vec![entry("extra-term", "từ bổ sung")];
