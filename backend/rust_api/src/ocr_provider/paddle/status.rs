...
fn stage_and_detail(raw_state: &str, state: &OcrTaskState) -> (&'static str, String) {
    match state {
OcrTaskState::Queued => ("ocr_upload", "Paddle has received task, waiting in queue".to_string()),
OcrTaskState::Running => ("ocr_processing", "Paddle is parsing file".to_string()),
