...
pub(super) fn record_bundle_retry_degraded(
    ...
    mark_ocr_result_ready(
        job,
        "OCR provider bundle dò liên tục bất thường, chuyển sang tải xuống trực tiếp theo chiến lược thử lại".to_string(),
    );
    register_job_retry(job);
    let mut payload = bundle_retry_payload(&event);
    ...
    record_custom_runtime_event_with_resources(
        ...
        "warn",
        "retry_degraded",
        "OCR provider bundle khả năng tiếp cận hạ cấp thành tải xuống trực tiếp",
        Some(payload),
    );
}
