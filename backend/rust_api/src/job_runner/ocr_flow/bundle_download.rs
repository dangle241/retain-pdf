...
pub(super) async fn download_and_unpack_after_success(
    ...
    mark_ocr_result_ready(
        job,
        "Kết quả OCR provider đã sẵn sàng, đang tải xuống bundle gốc".to_string(),
    );
    ...
    if !bundle_ready {
        ...
        mark_ocr_result_ready(
            job,
            "OCR provider bundle khả năng tiếp cận không ổn định, tiếp tục tải xuống thực tế để dự phòng".to_string(),
        );
        save_ocr_job(deps, job, parent_job_id).await?;
    }
