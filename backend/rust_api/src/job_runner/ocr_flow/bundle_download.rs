...
pub(super) async fn download_and_unpack_after_success(
    ...
    mark_ocr_result_ready(
        job,
        "OCR provider result is ready, downloading the raw bundle".to_string(),
    );
    ...
    if !bundle_ready {
        ...
        mark_ocr_result_ready(
            job,
            "OCR provider bundle reachability probe was not stable, falling back to real download".to_string(),
        );
        save_ocr_job(deps, job, parent_job_id).await?;
    }
