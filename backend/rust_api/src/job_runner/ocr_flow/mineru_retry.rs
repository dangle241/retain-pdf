...
"OCR provider upload address application failed. Retrying in {delay_secs}s (attempt {attempt}/{})",
                ...
                record_custom_runtime_event_with_resources(
                    ...
                    "warn",
                    "retry_scheduled",
                    "OCR provider Upload address application enters retry",
        ...
        match fetch().await {
            ...
            Err(err) => {
                ...
                if attempt >= runtime.poll_retry_limit {
                    ...
                    job.stage_detail =
                        Some("OCR provider Status query errors persist. Auto-retry later.".to_string());
                    job.updated_at = now_iso();
                    register_job_retry(job);
                    record_custom_runtime_event_with_resources(
                        deps.db.as_ref(),
                        ...
                        "warn",
                        "retry_scheduled",
                        format!("OCR provider {resource_label} Query downgraded; polling continues next round."),
                        Some(serde_json::json!({
