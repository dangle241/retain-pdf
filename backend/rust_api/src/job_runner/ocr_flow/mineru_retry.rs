            ...
            .await
        {
            ...
            Err(err) => {
                ...
                job.stage_detail = Some(format!(
                    "OCR provider upload URL request error, retrying in {delay_secs}s (attempt {attempt}/{})",
                ...
                record_custom_runtime_event_with_resources(
                    ...
                    "warn",
                    "retry_scheduled",
                    "OCR provider upload URL request entered retry",
        ...
        match fetch().await {
            ...
            Err(err) => {
                ...
                if attempt >= runtime.poll_retry_limit {
                    ...
                    job.stage_detail =
                        Some("OCR provider status query failed repeatedly, will continue polling automatically".to_string());
                    job.updated_at = now_iso();
                    register_job_retry(job);
                    record_custom_runtime_event_with_resources(
                        deps.db.as_ref(),
                        ...
                        "warn",
                        "retry_scheduled",
                        format!("OCR provider {resource_label} query downgraded to next polling round"),
                        Some(serde_json::json!({
                ...
                job.stage = Some("mineru_processing".to_string());
                job.stage_detail = Some(format!(
                    "OCR provider status query error, retrying in {delay_secs}s (attempt {attempt}/{})",
                    runtime.poll_retry_limit
                ));
                job.updated_at = now_iso();
                register_job_retry(job);
                record_custom_runtime_event_with_resources(
                    ...
                    "warn",
                    "retry_scheduled",
                    format!("OCR provider {resource_label} query entered retry"),
                    Some(serde_json::json!({
                        "scope": format!("mineru_{resource_label}_poll"),
                        "attempt": attempt,
                        "max_attempts": runtime.poll_retry_limit,
