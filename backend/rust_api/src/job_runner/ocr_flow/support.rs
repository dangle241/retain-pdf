...
async fn mirror_parent_ocr_status(
    ...
    if parent_stage_allows_ocr_mirror(parent_job.stage.as_deref()) {
        ...
        parent_job.stage_detail = ocr_job
            .stage_detail
            .as_ref()
            .map(|detail| format!("Tác vụ con OCR: {detail}"))
            .or_else(|| Some("Tác vụ con OCR đang chạy".to_string()));
...
pub(super) fn fail_missing_source_pdf(
    ...
    job.stage_detail = Some("OCR đã hoàn thành, nhưng tệp PDF nguồn của nhiệm vụ bị thiếu".to_string());
...
pub(super) fn fail_ocr_transport(job: &mut JobRuntimeState, err: &anyhow::Error) {
    ...
    if job
        ...
        .is_none()
    {
        job.stage_detail = Some("OCR provider transport thất bại".to_string());
    }
