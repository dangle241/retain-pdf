...
pub(super) fn attach_paddle_runtime_error(
    ...
    let info = OcrProviderErrorInfo {
        ...
        operator_hint: Some("Vui lòng kết hợp nhật ký job và trạng thái tác vụ Paddle để xử lý sự cố".to_string()),
        trace_id: job_artifacts_mut(job).provider_trace_id.clone(),
        http_status: None,
    };
    apply_paddle_error(job, info, format!("Paddle {stage} thất bại: {}", err));
    err
}
