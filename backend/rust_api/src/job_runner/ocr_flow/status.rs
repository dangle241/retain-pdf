...
fn ocr_stage_detail_with_progress(
    ...
    match (current, total) {
        (Some(current), Some(total)) if total > 0 => Some(format!(
            "Paddle đang phân tích tệp, trang thứ {}/{}",
            current.max(0),
            total
        )),
        (None, Some(total)) if total > 0 => Some(format!("OCR đang phân tích, tổng cộng {} trang", total)),
        _ => fallback,
    }
}
    ...
    fn ocr_stage_detail_prefers_page_progress_when_available() {
        assert_eq!(
            ocr_stage_detail_with_progress(
                Some("Paddle đang phân tích tệp".to_string()),
                Some(12),
                Some(34)
            )
            .as_deref(),
            Some("Paddle đang phân tích tệp, trang 12/34")
        );
        assert_eq!(
            ocr_stage_detail_with_progress(None, None, Some(34)).as_deref(),
            Some("OCR đang phân tích, tổng cộng 34 trang")
        );
    }
}
