...
pub(super) async fn run_local_ocr_transport_mineru(
    ...
    job.stage_detail = Some("OCR provider upload URL obtained, starting file upload".to_string());
    ...
    job.stage_detail = Some("File upload finished, waiting for OCR provider parsing".to_string());
    job.updated_at = now_iso();
    save_ocr_job(deps, job, parent_job_id).await?;
