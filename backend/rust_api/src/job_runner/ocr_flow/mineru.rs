...
pub(super) async fn run_local_ocr_transport_mineru(
    ...
    job.stage_detail = Some("Đã lấy địa chỉ tải lên OCR provider, bắt đầu tải tệp".to_string());
    ...
    job.stage_detail = Some("Tải tệp lên hoàn tất, đang chờ OCR provider phân tích".to_string());
    job.updated_at = now_iso();
    save_ocr_job(deps, job, parent_job_id).await?;
...
pub(super) async fn run_remote_ocr_transport_mineru(
    ...
    job.stage = Some("mineru_processing".to_string());
    job.stage_detail = Some("PDF từ xa đã gửi đến OCR provider, đang chờ phân tích".to_string());
    job.updated_at = now_iso();
    save_ocr_job(deps, job, parent_job_id).await?;
