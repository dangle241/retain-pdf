...
pub(super) fn classify_structured_failure(
    ...
    let summary = structured
        ...
        .clone()
        .filter(|value| !value.trim().is_empty())
        .unwrap_or_else(|| "Job failed, but no clear root cause was identified".to_string());
    ...
    let suggestion = structured
        ...
        .clone()
        .filter(|value| !value.trim().is_empty())
        .or_else(|| Some("Check the full traceback, raw exception, and log for further investigation".to_string()));
