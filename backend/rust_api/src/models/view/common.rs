...
pub struct ListJobsQuery {
    ...
    #[serde(default)]
    pub q: Option<String>,
    /// Comma-separated job_id Allowlist(Optional)Use it when category folder expands.
/// `GET /api/v1/documents?collection_id=` resolves active_job_id set
    /// convert to library card data——Behavior matches current state when omitted.
    #[serde(default)]
