//! Minimal library data layer. API:documents / favorites / full-text search.
//! Before frontend library refactor, existing /api/v1/library/books Projection interface unchanged.
//!
...
/// GET /api/v1/documents/:id/source.pdf — No translation job Reads source files too.
...
/// DELETE /api/v1/documents/:id â Permanently delete documentation (rows + jobs + uploads + files).
/// Referenced by favorites → 409;running job require ?force=true。
...
/// Reuse existing source from collection docs. PDF Initiate book Pipeline complete. lifecycle Backfills active_job_id。
