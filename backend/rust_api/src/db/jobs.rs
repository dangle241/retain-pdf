...
impl Db {
    ...
    pub fn recover_stale_running_job(
        ...
        let failure = JobFailureInfo {
            ...
            provider_stage: None,
            provider_code: None,
            summary: "Backend startup reclaimed leftover running jobs".to_string(),
            root_cause: Some(detail.to_string()),
            retryable: true,
            upstream_host: None,
            provider: None,
            suggestion: Some("The worker for this job is no longer running; please resubmit or manually retry".to_string()),
