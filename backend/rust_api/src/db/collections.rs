...
mod tests {
    ...
    fn add_remove_documents_tracks_document_count() {
        ...
        // collection_documents của document_id Yêu cầu chính của nước ngoài chứng minh sự tồn tại thực sự,Làm một cái trước。
        ...
        db.add_documents_to_collection("col-a", &[hash.clone()])
            .expect("add");
        // Idempotent:Lặp lại tham gia mà không có lỗi、Số lượng trùng lặp。
        db.add_documents_to_collection("col-a", &[hash.clone()])
            .expect("add again");
