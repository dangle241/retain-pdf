...
export function useLibraryController() {
  ...
  function downloadPdf(bookId: string) {
    ...
    downloadLibraryResource(downloadTarget, `${book?.title || bookId}.pdf`)
      ...
      .catch((error: unknown) => {
        feedback.setLoadError(error instanceof Error ? error.message : 'Failed to download PDF')
      })
      .finally(() => {
  ...
  function downloadArtifact(artifactKey: string) {
    ...
    downloadLibraryResource(artifact.downloadUrl, fallbackFileName)
      .then(() => {
        feedback.setToastText(`Download started ${artifact.label}`)
      })
      .catch((error: unknown) => {
        feedback.setLoadError(error instanceof Error ? error.message : 'Failed to download file')
      })
      .finally(() => {
