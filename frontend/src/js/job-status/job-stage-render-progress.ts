...
const compileText = compileDone ? "Rendering complete" : "Compiling PDF";
...
const prewarmText = `Prewarming ${record.current}/${record.total}`;
  const payload = (record.payload && typeof record.payload === "object")
    ? record.payload as Record<string, unknown>
    : {};
  return {
...
export function compositeRenderPrepareProgress(record: ProgressRecord | null | undefined) {
  ...
  return {
    ...
    progressUnit: "percent",
    displayPercent: percent,
progressText: `Preparing ${record.current}/${record.total}`,
    payload: {
      ...payload,
      progress_unit: "percent",
    },
