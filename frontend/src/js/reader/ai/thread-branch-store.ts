// assistant-ui branch树books地快照: 按 job 存全量 parentId 树 + headId.
...
export type ThreadBranchSnapshot = {
  ...
  /** 快照归属的会话 id(防串会话印章, 审计 P2-10)；旧快照None此字段 */
...
function normalizeMessage(raw: unknown): ThreadBranchMessage | null {
  ...
  // 不resume running: 刷新后不应卡在"Generating"
...
export function loadThreadBranchSnapshot(
  ...
  try {
    ...
    if (!raw && conversationId) {
      ...
      // 2. None印章的真旧快照, 只在"请求的正yesbooks job 的粘性会话"时才接受
      //    (旧快照写入时代唯一可能代表的就yes它).
