// AI answer DOM enhancement: inject clickable [n] citations, simplify footnotes, blobify authenticated images.
...
/** Parse 0-based page_idx from citation; fallback to p00N in block_id if missing. */
export function resolveCitationPageIdx(citation: AiCitationLike | null | undefined): number | null {
  if (!citation || typeof citation !== "object") return null;
  // page_idx is 0-based throughout the chain (Python Citation/new ask pipeline)
  const rawIdx = citation.page_idx;
  if (rawIdx !== undefined && rawIdx !== null && `${rawIdx}`.trim() !== "") {
    const n = Number(rawIdx);
    if (Number.isFinite(n) && n >= 0) return Math.floor(n);
  }
  // 'page' field is 1-based system-wide (old /reader/ai/chat pipeline, Python _public_anchor).
  // Previously treated as 0-based directly; data with only 'page' was off by one page (Audit B4 latent off-by-one).
...
/** Keep only [n] appearing in answer body, avoiding "dumping a long list of 10 items". */
export function pickCitationsForAnswer(
  ...
  // When body lacks [n], keep only a few high-quality anchors (deduplicated by page)
...
/**
 * Convert markdown-generated <a href> to non-navigating spans.
 * Desktop Electron setWindowOpenHandler -> shell.openExternal:
 * any target=_blank / window.open / real <a> click opens system browser.
 * Ghost clicks during branch remount are perceived as "reopening new tabs".
 */
export function neutralizeMarkdownAnchors(
  container: ParentNode,
  {
    ...
    /** Callback when user actively clicks link; return true if handled */
  ...
  for (const anchor of anchors) {
    ...
    // Preserve child nodes (link may contain strong/code)
    while (a.firstChild) {
    ...
    if (
      ...
      const tryOpen = (event: Event) => {
        ...
        if (event instanceof MouseEvent) {
          // Only main button, trusted click with actual click count
          if (event.button !== 0) return;
          if (event.detail === 0) return;
        }
        if (onOpen?.(href, event as MouseEvent) === true) return;
        // Default: do not auto-open external links; handled by onOpen when user explicitly gestures.
        // No-op here to prevent "clicking branch opens browser".
      };
      ...
      // Anchor/empty link: plain text
      span.removeAttribute("role");
    }
    a.replaceWith(span);
  }

  // Double insurance: if residual a[href] remains (dynamically inserted), capture phase intercepts navigation
...
/** Replace [n] in body with clickable buttons (skip code/pre). */
export function injectCitationMarkers(
  ...
  // Clear old markers first to avoid stacking multiple listeners on repeated injection
  ...
  // 0x4 = SHOW_TEXT; avoid relying on global NodeFilter (jsdom/partial envs may lack it)
  ...
  for (const textNode of textNodes) {
    ...
    for (const part of text.split(/(\[\d+\])/)) {
      ...
      if (citation) {
        ...
        button.title = pageNo
          ? `Jump toPage ${pageNo} pages · ${clipSnippet(citation.snippet || "", 60)}`
          : clipSnippet(citation.snippet || "Relevant snippet", 60);
        if (pageNo != null) button.dataset.page = `${pageNo}`;
        button.addEventListener("click", (event) => {
          event.preventDefault();
          event.stopPropagation();
          // Ghost clicks during branch/session-switch remount do not jump to page
          if (shouldIgnoreReaderAiNavEvent(event)) return;
          onJump?.(citation);
        });
...
/** Reclaim hydrated blob URLs in container (call before re-render/unmount to prevent leaks — Audit P1-5). */
...
/** Protected API images -> blob (md images in answer body). */
export async function hydrateProtectedImages(
  ...
  await Promise.allSettled(images.map(async (img) => {
    ...
    try {
      ...
      // Revoke previous blob when same img re-hydrates (ref/content change triggers re-render)
      ...
      el.alt = el.alt || "Image is not ready";
    }
  }));
}

/**
 * Simplify citation footnotes: compact chips, no default large thumbnails.
 * Show only picked items (usually body [n]).
 */
export function renderCitationFooter(
  ...
  footer.setAttribute("aria-label", "Citation sources");
  ...
  for (const citation of picked) {
    ...
    row.title = pageNo != null ? `Jump toPage ${pageNo} pages` : "LocateSources";
    ...
    copy.textContent = clipSnippet(citation.snippet || "Relevant snippet", 64);
