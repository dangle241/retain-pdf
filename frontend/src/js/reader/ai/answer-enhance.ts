// AI No source text provided. DOM Enhancement: Injection [n] Clickable citations, concise footnotes, authenticated images blob Normalize.
...
/** Parse 0-based page_idx from citation; default try p00N in block_id. */
export function resolveCitationPageIdx(citation: AiCitationLike | null | undefined): number | null {
  if (!citation || typeof citation !== "object") return null;
// page_idx Full chain 0 Base (Python Citation/new ask Link
  const rawIdx = citation.page_idx;
  if (rawIdx !== undefined && rawIdx !== null && `${rawIdx}`.trim() !== "") {
    const n = Number(rawIdx);
    if (Number.isFinite(n) && n >= 0) return Math.floor(n);
  }
  // page All fields within system 1 Base (legacy) /reader/ai/chat Link.Python _public_anchor）
  // ——Previously treated as 0 Use directly. Only carry. page Audit data off by one page. B4 latent off-by-one）
...
/** Keep only content present in the response body. [n], avoid "dumping 10 Long lists". */
export function pickCitationsForAnswer(
  ...
  // Body unmarked [n] Retain few high-quality anchors (deduplicate per page).
...
/**
* Change markdown generated <a href> to navigationless span.
* Desktop Electron setWindowOpenHandler â shell.openExternal:
* Any target=_blank / window.open / real <a> click opens system browser.
* Branch remount Ghost clicks detected as body sensation. "Reopen new tab".
 */
export function neutralizeMarkdownAnchors(
  container: ParentNode,
  {
    ...
    /** Callback on user-initiated link click; return true Mark processed */
  ...
  for (const anchor of anchors) {
    ...
    // Keep child nodes (links may contain strong/code）
    while (a.firstChild) {
    ...
    if (
      ...
      const tryOpen = (event: Event) => {
        ...
        if (event instanceof MouseEvent) {
          // Primary button only. Credible clicks with actual click count.
          if (event.button !== 0) return;
          if (event.detail === 0) return;
        }
        if (onOpen?.(href, event as MouseEvent) === true) return;
// Default: off. openExternal requires explicit user gesture. Handled in onOpen.
        // Only do here no-opAvoid any「Clicking branch opens browser.」。
      };
...
// Anchor/Empty chain: plain text
      span.removeAttribute("role");
    }
    a.replaceWith(span);
  }

  // Double-check: if residues remain a[href]Dynamic insertion; capture phase intercepts all navigation.
...
/** Body text. [n] Convert to clickable button (Skip) code/pre）。 */
export function injectCitationMarkers(
  ...
  // Clear old markers to prevent duplicates. inject Stack listeners
  ...
  // 0x4 = SHOW_TEXTRemove dependencies → skipped: additional libraries, add when absolutely necessary. NodeFilter Globaljsdom/some environments not mounted)
  ...
  for (const textNode of textNodes) {
    ...
    for (const part of text.split(/(\[\d+\])/)) {
      ...
      if (citation) {
        ...
        button.title = pageNo
? `Go to page ${pageNo} Â· ${clipSnippet(citation.snippet || "", 60)}`
          : clipSnippet(citation.snippet || "Related Snippets", 60);
        if (pageNo != null) button.dataset.page = `${pageNo}`;
        button.addEventListener("click", (event) => {
          event.preventDefault();
          event.stopPropagation();
// Branch/Switch session remount Ghost clicks during period don't navigate.
          if (shouldIgnoreReaderAiNavEvent(event)) return;
          onJump?.(citation);
        });
...
/** Recycle containers already hydrated blob URL re-renders. Call before uninstall to prevent leaks. (Audit P1-5). */
...
/** Protected API Image → blobin the answer body md Figure). */
...
// Same img repeat hydrate (reference/Reclaim old instances on content-change re-render. blob
      ...
      el.alt = el.alt || "Image temporarily unavailable.";
    }
  }));
}

/**
 * Compact citation footnotes chipDefault thumbnail not full-size.
 * Display only pick Subsequent entries (typically body [n]）。
 */
export function renderCitationFooter(
  ...
  footer.setAttribute("aria-label", "Cite Source");
...
row.title = pageNo != null ? `Route wrapper: page ${pageNo}` : "Location Source";
    ...
    copy.textContent = clipSnippet(citation.snippet || "relevant segments", 64);
