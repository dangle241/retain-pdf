...
function buildAnswer(question, sections) {
  if (!sections.length) {
    return "Tôi không tìm thấy đủ đoạn liên quan trong Markdown hiện tại. Hãy thử một câu hỏi cụ thể hơn hoặc xác nhận tác vụ đã tạo Markdown.";
  }
  const lines = [
    "Trước tiên, tôi tìm thấy các đoạn liên quan sau trong Markdown hiện tại:",
...
export function createReaderMarkdownAnswerer({
  ...
  async function answer({ jobId = "", question = "", scope = "document", context = null }: any = {}) {
    ...
    if (!source) {
      throw new Error("Tác vụ hiện tại chưa có Markdown dùng được cho hỏi đáp.");
    }
