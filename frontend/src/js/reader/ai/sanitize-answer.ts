// Sanitize internal model output. block_id Reference and map to [n]。
...
// Code protection: fence blocks (including unclosed streaming tails) + inline code. Extract whole paragraph.
// Prevent collapse by whitespace./block_id Sanitize——Python Indentation, table alignment collapsed to single line (audit P1-6）
const CODE_SEGMENT_RE = /```[\s\S]*?(?:```|$)|`[^`\n]+`/g;
const CODE_SLOT_PREFIX = "CODE_";
const CODE_SLOT_SUFFIX = "";

/** Replace [p002-b0004] / bare p002-b0004 with [n]; delete if unmappable. Keep code blocks as is. */
export function sanitizeAssistantAnswer(
  ...
  // Remove residual internal field verbal tics.
