...
export function disabledReason(action, urls) {
  if (action === "sideBySide" && (!urls.source || !urls.translated)) {
    return "PDF đối chiếu yêu cầu cả PDF gốc và PDF bản dịch đều khả dụng";
  }
  if (!urls.source && (action === "source" || action === "sideBySide")) {
    return "PDF gốc chưa được tạo hoặc danh sách đầu ra không khả dụng";
  }
  if (!urls.translated && (action === "translated" || action === "sideBySide")) {
    return "PDF bản dịch chưa được tạo hoặc danh sách đầu ra không khả dụng";
  }
  return "Địa chỉ tải xuống tạm thời không khả dụng";
}
