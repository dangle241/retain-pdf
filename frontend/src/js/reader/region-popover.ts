...
export function formatReaderRegionMarkdownPayload(payload) {
  ...
  return {
    ...
    primaryLabel: markdown ? "Markdown" : (translated ? "Bản dịch" : "Nguyên văn"),
    primaryText: markdown || translated || source || "Vùng này chưa có văn bản để hiển thị",
    source,
    translated,
  };
}
...
export function renderReaderMarkdownPopover(event, region, state) {
  ...
  popover.innerHTML = `
    ...
    <div class="reader-region-markdown-body">${escapeHtml(state?.message || "Đang tải...")}</div>
  `;
...
export function renderReaderMarkdownPayload(popover, payload) {
  ...
  popover.querySelector(".reader-region-markdown-body").innerHTML = `
    ...
    ${formatted.source && formatted.primaryText !== formatted.source ? renderReaderTextBlock("Nguyên văn", formatted.source) : ""}
  `;
  return formatted;
}
