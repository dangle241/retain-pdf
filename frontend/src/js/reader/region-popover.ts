...
export function renderReaderMarkdownPopover(event, region, state) {
  ...
  popover.innerHTML = `
    <div class="reader-region-markdown-head">
      ...
      <button type="button" class="reader-region-markdown-close" aria-label="Close">×</button>
    </div>
    <div class="reader-region-markdown-body">${escapeHtml(state?.message || "Loading...")}</div>
  `;
...
export function renderReaderMarkdownPayload(popover, payload) {
  ...
  popover.querySelector(".reader-region-markdown-body").innerHTML = `
    ...
    ${formatted.source && formatted.primaryText !== formatted.source ? renderReaderTextBlock("Source", formatted.source) : ""}
  `;
  return formatted;
}
