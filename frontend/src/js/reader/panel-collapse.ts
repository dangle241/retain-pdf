// Collapse left/right panes in three-column skeleton.:Toggle hand body Collapse class above,Collapse this column(Width reset to zero.),State persistence.
// Collapse is purely visual.(Skip right column. active Drawer),Original content persists after expansion.
...
// Quota full/Privacy mode: silent failure
  }
}
...
export function createReaderPanelCollapse({
  ...
  function apply() {
    ...
    // aria-expanded Feedback"Column expanded?";Handle Icon is-collapsed Flip(CSS)
  ...
  // Tool opens from top bar.,Auto-expand right column if collapsed to reveal content.
