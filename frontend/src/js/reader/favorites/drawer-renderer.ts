...
export function renderFavorites(listEl, items = [], {
  ...
  if (!items.length) {
    ...
    empty.innerHTML = [
      ...
      '<p class="empty-state-hint">Chọn một đoạn hoặc biểu đồ rồi bấm “Lưu”; trích đoạn sẽ xuất hiện tại đây.</p>',
    ].join("");
  ...
  items.forEach((item) => {
    ...
    locateButton.setAttribute("aria-label", "Định vị ảnh chụp");
