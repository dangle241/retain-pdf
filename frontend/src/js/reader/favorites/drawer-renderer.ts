...
export function renderFavorites(listEl, items = [], {
  ...
  if (!items.length) {
    ...
    empty.innerHTML = [
      ...
      '<p class="empty-state-title">No screenshots</p>',
'<p class="empty-state-hint">Click after selecting a paragraph or chart. "Favorite" excerpts will appear here.</p>',
    ].join("");
  ...
  items.forEach((item) => {
    ...
    locateButton.setAttribute("aria-label", "Locate Screenshot Position");
    ...
    removeButton.setAttribute("aria-label", "Delete screenshot excerpt");
