...
export function renderFavorites(listEl, items = [], {
  ...
  if (!items.length) {
    ...
    empty.innerHTML = [
      ...
      '<p class="empty-state-title">No Clipped excerpt</p>',
      '<p class="empty-state-hint">Select a paragraph or figure and click Favorite. Excerpts will appear here.</p>',
    ].join("");
  ...
  items.forEach((item) => {
    ...
    locateButton.setAttribute("aria-label", "Locate clipped excerpt");
    ...
    removeButton.setAttribute("aria-label", "Delete clipped excerpt");
    ...
    meta.textContent = `Page ${item.page || "-"} pages`;
