...
function formatSelection(selection: ReaderAiSelectionContext | ReaderSelection = {}) {
  ...
  return `Page ${selection.page || "-"} pages · ${width} × ${height}`;
}
...
export function createReaderAiContext({
  ...
  function sync() {
    ...
    if (scope === "selection" && selection) {
      contextEl.textContent = `Current selection: ${formatSelection(selection)}`;
      return;
    }
    if (scope === "page" && selection?.page) {
      contextEl.textContent = `Current page: Page ${selection.page} pages`;
      return;
    }
    contextEl.textContent = "Current scope: entire document";
  }
