...
export function buildMockStageHistory(scenario: string): StageHistoryEntry[] {
  const stages = [
    { key: "queued", detail: "Tải PDF lên", duration_ms: scenario === "upload" ? null : 18_000 },
    { key: "ocr_processing", detail: "Phân tích OCR", duration_ms: scenario === "ocr" ? null : 126_000 },
    { key: "translating", detail: "Dịch nội dung", duration_ms: scenario === "translate" ? null : 214_000 },
    { key: "rendering", detail: "Kết xuất PDF", duration_ms: scenario === "render" || scenario === "failed" ? null : 74_000 },
    { key: "finished", detail: "Phát hành đầu ra", duration_ms: scenario === "done" ? 28_000 : null },
  ];
...
export function buildMockJobPayload(scenario = currentMockScenario()): JobLike {
  ...
  const scenarioMap: Record<string, {
    ...
    upload: {
      ...
      total: 12,
      percent: 17,
      stageDetail: "Đang tải PDF lên và chuẩn bị gửi tác vụ OCR",
      activeMs: 18_000,
      totalMs: 18_000,
    },
    ocr: {
    ...
    translate: {
      ...
      total: 55,
      percent: 33,
      stageDetail: "Đang dịch nội dung và công thức, đợt 18/55",
      activeMs: 214_000,
      totalMs: 358_000,
    },
    parallel: {
      ...
      total: 900,
      percent: 13,
      stageDetail: "Đang dịch nội dung, đợt 120/900",
      activeMs: 236_000,
      totalMs: 380_000,
      backgroundStages: [
        {
    ...
    render: {
      ...
      total: 12,
      percent: 67,
      stageDetail: "Đang kết xuất trang 8/12",
      activeMs: 74_000,
      totalMs: 512_000,
    },
    done: {
      ...
      total: 12,
      percent: 100,
      stageDetail: "Xử lý hoàn tất, có thể tải kết quả xuống",
      activeMs: 28_000,
      totalMs: 540_000,
    },
    failed: {
      ...
      total: 12,
      percent: 75,
      stageDetail: "Giai đoạn kết xuất thất bại",
      activeMs: 96_000,
      totalMs: 496_000,
    },
  };
