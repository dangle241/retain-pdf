...
export function getMockJobPayload(jobId = ""): JobLike {
  const id = `${jobId || ""}`.trim();
  // Job live tạo khi gửi dịch: tiến triển theo đồng hồ thật (hoạt ảnh tiến độ tab chi tiết).
  const live = buildLiveMockJobPayload(id);
  if (live) {
    return enrichJobWithDocument(live, id) as JobLike;
  }
  // Job mock chính: theo tình huống URL ?mock=.
  if (!id || id === MOCK_JOB_ID) {
    return enrichJobWithDocument(buildMockJobPayload(), id || MOCK_JOB_ID) as JobLike;
  }
  // active_job_id tổng hợp từ trung tâm tài liệu (như 20260520-att-001): trả payload trạng thái cuối đầy đủ,
  // để StatusCard nhúng trong chi tiết sách lấy dữ liệu cùng hình dạng với job thành công thật (luồng giai đoạn/đầu ra sẵn sàng).
  ...
  return {
    ...
    status: "succeeded",
    stage: "finished",
    stage_detail: book.stage_detail || "Tác vụ hoàn tất",
  } as JobLike;
}
...
// Lưới trung tâm tài liệu (F2) dùng library/books?job_ids= để lấy hàng loạt trạng thái sống của "tài liệu mock đã dịch".
// Ưu tiên lấy tên sách/bìa thật từ bảng mock; không dùng job_id.pdf làm tiêu đề nữa (một nguyên nhân bìa trống).
...
export function submitMockJob(): JobLike {
  // Luồng tải lên "Bắt đầu dịch" cũng dùng tác vụ live để thấy hoạt ảnh tiến triển trong vùng trạng thái.
  const live = registerLiveMockJob({
    title: "Mô phỏng tải lên và dịch",
    pageCount: 12,
  });
