...
export function buildMockEvents(scenario = currentMockScenario()) {
  const items: any[] = [
    {
      ...
      level: "info",
      stage: "queued",
      stage_detail: "Đã tải PDF lên, tác vụ đã vào hàng đợi",
      event_type: "stage_progress",
      event: "stage_progress",
      message: "Đã tải PDF lên, tác vụ đã vào hàng đợi",
      progress_current: 2,
      progress_total: 12,
      payload: { scenario },
    },
  ...
  if (["ocr", "translate", "render", "done", "failed"].includes(scenario)) {
    items.push({
      ...
      event_type: "stage_progress",
      event: "stage_progress",
      message: "Đang thực hiện OCR, trang 5/12",
  ...
  if (["translate", "parallel", "render", "done", "failed"].includes(scenario)) {
    ...
    items.push({
      ...
      stage_detail: "Đang dịch nội dung và công thức, đợt 18/55",
      event_type: "progress",
      event: "stage_progress",
      message: "Đang dịch nội dung và công thức, đợt 18/55",
  ...
  if (scenario === "parallel") {
    items.push({
      ...
      stage_detail: "Backend đang khởi động trước tài nguyên kết xuất",
      event_type: "progress",
      event: "stage_progress",
  ...
  if (["render", "done", "failed"].includes(scenario)) {
    items.push({
      ...
      level: "info",
      stage: "rendering",
      stage_detail: scenario === "failed" ? "Đang kết xuất trang 9/12" : "Đang kết xuất trang 8/12",
      event_type: "stage_progress",
      event: "stage_progress",
      message: scenario === "failed" ? "Đang kết xuất trang 9/12" : "Đang kết xuất trang 8/12",
  ...
  if (scenario === "done") {
    items.push({
      ...
      level: "info",
      stage: "finished",
      stage_detail: "Đã tạo PDF, có thể tải xuống",
      event_type: "artifact_published",
      event: "artifact_published",
      message: "Đã tạo PDF, có thể tải xuống",
      progress_current: 12,
      progress_total: 12,
  ...
  if (scenario === "failed") {
    items.push({
      ...
      event: "job_failed",
      message: "Giai đoạn kết xuất thất bại",
      progress_current: 9,
      progress_total: 12,
