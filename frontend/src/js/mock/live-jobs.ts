// Tác vụ dịch mock có thể tiến triển: sau khi gửi sẽ hoàn thành các giai đoạn theo đồng hồ thật.
// Hỗ trợ fromStage: khi bấm "OCR/dịch/kết xuất lại", chạy từ giai đoạn đó và coi các giai đoạn trước đã hoàn tất.
...
export type LiveMockJobMeta = {
  ...
  pageCount?: number;
  startedAtMs: number;
  /** Tỷ lệ tổng thời lượng (1 = mặc định khoảng 16 giây cho toàn luồng; có fromStage thì chỉ chạy phần còn lại). */
  speed?: number;
  /**
   * Bắt đầu tiến triển từ giai đoạn nào.
   * - Bỏ qua / upload: toàn bộ pipeline
   * - ocr: bỏ xếp hàng, bắt đầu từ OCR
   * - translate: bỏ xếp hàng + OCR, bắt đầu từ dịch
   * - render: chỉ chạy kết xuất
   */
...
const PHASES: PhaseDef[] = [
  ...
  },
  {
    ...
    unit: "batch",
    total: 40,
    detail: ({ current, total }) => `Đang dịch nội dung, đợt ${current}/${total}`,
  },
  {
    key: "render",
    status: "running",
    ...
    unit: "page",
    total: 12,
    detail: ({ current, total }) => `Đang kết xuất trang ${current}/${total}`,
  },
  {
    key: "done",
    status: "succeeded",
    ...
    unit: "none",
    total: 1,
    detail: () => "Xử lý hoàn tất, có thể đọc đối chiếu",
  },
];
...
/** Tên stage API → pha from của pipeline (done không làm điểm bắt đầu). */
...
/**
 * Tạo dòng thời gian: các pha trước fromStage có duration=0 (hoàn tất tức thì),
 * từ fromStage trở đi tiến triển theo thời lượng bình thường.
 */
function phaseTimeline(speed = 1, fromStage?: LiveMockFromStage | PhaseKey | string) {
  ...
  return PHASES.map((phase, index) => {
    // done luôn nằm cuối; các pha không phải done trước from bỏ qua thời lượng.
...
/**
 * Đăng ký một tác vụ có thể tiến triển. Trả về job_id.
 * fromStage: entry thử lại, bắt đầu từ ocr / translate / render.
 */
...
export function buildLiveMockJobPayload(
  ...
  const title = `${meta.title || "Tác vụ dịch mô phỏng"}`.trim() || "Tác vụ dịch mô phỏng";

  // Lịch sử: các pha trước fromStage được đánh dấu hoàn tất; pha hiện tại và sau đó theo thời gian.
  ...
  return {
    ...
    lane: "main",
    stage_detail: isDone ? "Xử lý hoàn tất, có thể đọc đối chiếu" : detail,
    progress: {
