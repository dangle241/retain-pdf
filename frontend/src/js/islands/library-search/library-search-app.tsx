...
function SearchHit({ hit, onOpenReader }) {
  return (
    <button
      ...
      title={`Page ${Number(hit.page_idx) + 1} pages · ${hit.block_id}`}
    >
      <Snippet text={hit.source_snippet} />
      {hit.translated_snippet ? <Snippet text={hit.translated_snippet} /> : null}
      <span className="lib-search-hit-meta">Page {Number(hit.page_idx) + 1} pages</span>
    </button>
  );
}
    ...
    <div className="lib-search-doc">
      ...
      >
        ...
        <span className="lib-search-doc-meta">
          {doc.page_count} pages{doc.tags.length ? ` · ${doc.tags.join(" / ")}` : ""}
        </span>
      </button>
      <button
        type="button"
  ...
  return (
    <div className="lib-search-panel" role="region" aria-label="Library search results">
      <div className="lib-search-head">
        ...
        <span className="lib-search-status">{busy ? "Searching..." : error || `${hits.length} full-text hits · ${matchedDocuments.length} documents`}</span>
        <div className="lib-search-filters" role="group" aria-label="Filter by reading status">
