  ...
  return (
    <div className="lib-search-doc">
      ...
      <button
        ...
        title="Bấm để đổi trạng thái đọc"
      >
        {meta.label}
      </button>
    </div>
...
function LibrarySearchPanel({ ports }) {
  ...
  return (
    <div className="lib-search-panel" role="region" aria-label="Kết quả tìm kiếm trong thư viện">
      <div className="lib-search-head">
        <strong>Tìm trong thư viện</strong>
        <span className="lib-search-status">{busy ? "Đang tìm kiếm…" : error || `${hits.length} kết quả toàn văn · ${matchedDocuments.length} tài liệu`}</span>
      ...
      <section className="lib-search-section">
        ...
        {matchedDocuments.length === 0
          ? <p className="lib-search-empty">Không có tài liệu phù hợp</p>
          : (
