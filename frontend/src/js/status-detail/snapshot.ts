...
function statusDetailNote(job: StatusDetailJob = {}): string {
  return job.status === "failed"
    ? "Xem nguyên nhân lỗi, đề xuất và luồng sự kiện"
    : job.status === "succeeded" && isJobTerminal(job)
      ? "Tác vụ đã hoàn tất; có thể xem tổng quan và luồng sự kiện"
      : "Xem tổng quan tác vụ, nguyên nhân lỗi và luồng sự kiện";
}
...
function summarizeMathMode(job: StatusDetailJob): string {
  ...
  if (mathMode === "placeholder") {
    return "placeholder - bảo vệ chỗ giữ chỗ của công thức";
  }
  if (mathMode === "direct_typst") {
    return "direct_typst - mô hình xuất công thức Typst trực tiếp";
  }
  return mathMode || "-";
}
...
export function buildStatusDetailSnapshot(
  ...
  return {
    ...
    rerun: {
      ...
      status: rerunEnabled
        ? "Backend hỗ trợ tạo tác vụ khôi phục từ đầu ra của tác vụ hiện tại."
        : "Tác vụ hiện tại chưa thể tiếp tục từ điểm dừng.",
    },
  };
}
