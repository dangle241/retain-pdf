...
function stageIconMarkup(job: StatusDetailJob, stageText: string | undefined): string {
  ...
  if (text.includes("parse") || text.includes("ocr")) {
...
function statusDetailNote(job: StatusDetailJob = {}): string {
  return job.status === "failed"
    ? "View failure reason, suggestion, and event stream"
    : job.status === "succeeded" && isJobTerminal(job)
      ? "Job complete. Overview and event stream are ready to view"
      : "View job overview, failure reason, and event stream";
}
...
function buildFailureDetails(job: StatusDetailJob) {
  ...
  return {
    ...
    retryable: typeof (diagnostics.retryable ?? retryable) === "boolean" ? ((diagnostics.retryable ?? retryable) ? "yes" : "no") : "-",
  };
}
...
export function buildStatusDetailSnapshot(
  ...
  return {
    ...
    rerun: {
      ...
      status: rerunEnabled
        ? "The backend can create a resumed job from current job artifacts."
        : "The current job cannot resume from a checkpoint.",
    },
  };
}
