...
export async function loadAndRenderMarkdownFlow({
  ...
  try {
    ...
    if (markdownPayload) {
      ...
      setText("detail-markdown-status", "Markdown đã được đánh dấu ready nhưng /markdown chưa trả về nội dung");
    }
  } catch (error) {
    renderMarkdownContract({
      job,
    ...
    setText("detail-markdown-status", error.message || "Không thể tải Markdown");
  }
}
