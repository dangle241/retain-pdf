...
function summarizeArtifactLabel(key) {
  switch (`${key || ""}`.trim()) {
    ...
    case "typst_render_pdf":
      return "PDF kết xuất bằng Typst";
    case "markdown_raw":
      return "Markdown Raw";
...
export function renderArtifactsManifest(manifestPayload) {
  ...
  summary.textContent = items.length > 0 ? `Tổng cộng ${items.length} mục` : "Chưa có đầu ra nào được đăng ký";
  if (items.length === 0) {
    container.innerHTML = '<div class="detail-empty">Chưa có danh sách đầu ra</div>';
    return;
  }
...
export function renderMarkdownContract({
  ...
  if (!contract.ready) {
    ...
    setText("detail-markdown-status", "Tác vụ hiện tại chưa có Markdown đã phát hành");
  ...
  if (!markdownPayload) {
    setText("detail-markdown-status", "Đã phát hành, đang tải nội dung…");
    return;
  }
  ...
  const statusBits = [markdownPayload?.content_with_absolute_image_urls ? "Đã tải /markdown/document" : "Đã tải JSON /markdown"];
  if (fileName) {
...
export async function renderMarkdownImagePreview({
  ...
  const previews = await Promise.all(previewRefs.map(async (ref) => {
    ...
    try {
      ...
      return { ref, absoluteUrl, objectUrl: "", error: error.message || "Không thể tải ảnh" };
    }
  }));
