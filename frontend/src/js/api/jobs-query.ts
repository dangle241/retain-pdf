...
export async function fetchJobPayload(jobId, apiPrefix) {
  ...
  if (!resp.ok) {
    if (resp.status === 404) {
      throw new Error("Không tìm thấy tác vụ, vui lòng kiểm tra job_id.");
    }
    throw new Error(`Không thể tải tác vụ, vui lòng thử lại sau. (${resp.status})`);
  }
...
export async function fetchJobList(
  ...
  if (!resp.ok) {
    throw new Error(`Không thể tải các tác vụ gần đây, vui lòng thử lại sau. (${resp.status})`);
  }
  return unwrapEnvelope(await resp.json());
}
