...
export async function submitJson(url, payload) {
  ...
  if (!resp.ok) {
    ...
    if (contentType.includes("application/json")) {
      ...
      throw new Error(`Gửi yêu cầu thất bại: ${resp.status} ${errorPayload.message || JSON.stringify(errorPayload)}${requestContext}`);
    }
    const text = await resp.text();
    throw new Error(`Gửi yêu cầu thất bại: ${resp.status} ${text}${requestContext}`);
  }
  if (resp.status === 204) {
    return { ok: true };
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    xhr.addEventListener("load", () => {
      ...
      const message = typeof xhr.response === "object" && xhr.response
        ...
        : (xhr.responseText || "");
      const error = new Error(`Gửi yêu cầu thất bại: ${xhr.status} ${message}`) as HttpError;
    ...
    xhr.addEventListener("error", () => {
      const error = new Error(`Gửi yêu cầu thất bại: lỗi mạng. API Base hiện tại là ${apiBase()}, địa chỉ tải lên là ${url}. Hãy xác nhận dịch vụ cục bộ đã khởi động.`) as HttpError;
      error.url = url;
      reject(error);
    });
