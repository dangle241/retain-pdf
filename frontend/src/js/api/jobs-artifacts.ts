...
export async function fetchJobArtifactsManifest(jobId, apiPrefix) {
  ...
  if (!resp.ok) {
    ...
    throw new Error(`Không thể tải danh sách đầu ra, vui lòng thử lại sau. (${resp.status})`);
  }
...
export async function fetchJobMarkdown(jobId, apiPrefix) {
  ...
  if (!resp.ok) {
    ...
    throw new Error(`Không thể tải Markdown, vui lòng thử lại sau. (${resp.status})`);
  }
  return unwrapEnvelope(await resp.json());
}
...
export async function fetchJobMarkdownDocument(jobId, apiPrefix) {
  ...
  if (!resp.ok) {
    ...
    throw new Error(`Không thể tải Markdown có cấu trúc, vui lòng thử lại sau. (${resp.status})`);
  }
  return unwrapEnvelope(await resp.json());
}
