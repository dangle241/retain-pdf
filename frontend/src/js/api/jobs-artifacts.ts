...
export async function fetchJobArtifactsManifest(jobId, apiPrefix) {
  ...
  if (!resp.ok) {
    ...
    throw new Error(`Failed to read artifact list, please try again later.(${resp.status})`);
  }
...
throw new Error(Failed to read structured Markdown, please try again later. (${resp.status}));
  }
  return unwrapEnvelope(await resp.json());
}
