...
export async function listCollections(apiPrefix) {
  ...
  if (!resp.ok) {
    throw new Error(`Failed to read category, please try again later.(${resp.status})`);
  }
...
export async function createCollection(apiPrefix, { name, parentId = "" }: any = {}) {
  ...
  if (!resp.ok) {
    ...
    throw new Error(`${envelope?.message || "Failed to create category. Please try again later."}(${resp.status})`);
  }
...
throw new Error("Missing collection_id.");
  ...
  if (!resp.ok) {
    const envelope = await resp.json().catch(() => null);
    throw new Error(`${envelope?.message || "Update category failed. Try again later."}(${resp.status})`);
  }
  return unwrapEnvelope(await resp.json());
}
...
throw new Error("Missing collection_id.");
  ...
  if (!resp.ok) {
    const envelope = await resp.json().catch(() => null);
    throw new Error(`${envelope?.message || "Delete category failed. Try again later."}(${resp.status})`);
  }
  return unwrapEnvelope(await resp.json());
}
...
throw new Error("Missing collection_id.");
  ...
  if (!resp.ok) {
    const envelope = await resp.json().catch(() => null);
    throw new Error(`${envelope?.message || "Failed to join category. Please try again later."}(${resp.status})`);
  }
  return unwrapEnvelope(await resp.json());
}
...
throw new Error("Missing collection_id or document_id.");
  ...
  if (!resp.ok) {
    const envelope = await resp.json().catch(() => null);
    throw new Error(`${envelope?.message || "Failed to move out of category, please try again later."}(${resp.status})`);
  }
  return unwrapEnvelope(await resp.json());
}
