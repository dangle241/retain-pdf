...
export async function fetchTranslationDiagnostics(jobId, apiPrefix) {
  ...
  if (!resp.ok) {
    if (resp.status === 404) {
      throw new Error("Translation debug info not found. Confirm task translation completed.");
    }
...
export async function fetchTranslationItem(jobId, itemId, apiPrefix) {
  ...
  if (!resp.ok) {
    if (resp.status === 404) {
throw new Error("Translation item not found. Please confirm item_id is correct.");
    }
    throw new Error(`Read translation item Failed to fetch details, please try again later.(${resp.status})`);
  }
  return unwrapEnvelope(await resp.json());
}
