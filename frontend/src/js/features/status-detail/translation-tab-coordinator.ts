export function createStatusDetailTranslationTabCoordinator({
  ...
  function renderSelectionPlaceholder(selection) {
    renderItemDetail({
      emptyText: selection?.selectedItemId ? "Vui lòng chọn một mục bên trái" : "Không có mục nào để xem",
    });
    renderReplay();
  }
  ...
  async function ensureLoaded({ force = false }: any = {}) {
    ...
    renderEmpty("Đang tải dữ liệu gỡ lỗi bản dịch...");
    try {
