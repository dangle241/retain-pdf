...
export function summarizeResumePlan(plan) {
  ...
  if (!plan.can_resume) {
    return plan.reason || "Tác vụ hiện tại chưa thể tiếp tục từ điểm dừng.";
  }
...
export function syncRerunAction({
  ...
  viewPort.setRerunAction({
    ...
    status: statusText || (enabled
      ? summarizeResumePlan(resumePlan) || "Backend hỗ trợ tạo tác vụ khôi phục từ đầu ra của tác vụ hiện tại."
      : summarizeResumePlan(resumePlan) || "Tác vụ hiện tại chưa thể tiếp tục từ điểm dừng."),
  });
...
export async function rerunCurrentJob({
  ...
  if (!actionUrl) {
    syncRerunAction({
      ...rerunContext,
      statusText: "Tác vụ hiện tại chưa thể tiếp tục từ điểm dừng.",
      viewPort,
      resolveActions,
    });
    return;
  ...
  try {
    ...
    setText?.("error-box", `Đã tạo tác vụ khôi phục ${nextJobId}, bắt đầu thăm dò trạng thái.`);
