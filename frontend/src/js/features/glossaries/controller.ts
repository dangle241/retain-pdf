...
export function mountGlossariesFeature({
  ...
  async function selectGlossary(glossaryId) {
    ...
    renderList();
    viewPort.setStatus("Loading glossary...");
    try {
  ...
  function createNew() {
    ...
    viewPort.setStatus("New glossary has not been saved yet.");
  }

  async function save() {
    const payload = viewPort.readEditorPayload();
    if (!payload.name.trim()) {
      viewPort.setStatus("Enter a glossary name.", "error");
      return;
    }
    if (payload.skippedMissingTarget?.length > 0) {
      viewPort.setStatus("Fixed translation/preferred translations require a translation value.", "error");
      return;
    }
    ...
    try {
      ...
      viewPort.setStatus("Saved.", "valid");
    } catch (err) {
      viewPort.setStatus(err.message || String(err), "error");
    }
  }
  ...
  async function exportCurrent() {
    if (!state.selectedId || state.draftOnly) {
      viewPort.setStatus("Save the glossary before exporting.", "error");
      return;
    }
    if (typeof exportGlossaryCsv !== "function") {
      viewPort.setStatus("Glossary export is not connected in the current environment.", "error");
      return;
    }
  ...
  async function applyImport() {
    ...
    if (!csvText.trim()) {
      viewPort.setStatus("Paste CSV content first.", "error");
      return;
    }
    ...
    try {
      ...
      viewPort.setStatus(`Parsed ${Number(payload?.entry_count) || 0} entries.`, "valid");
    } catch (err) {
      viewPort.setStatus(err.message || String(err), "error");
    }
  }
