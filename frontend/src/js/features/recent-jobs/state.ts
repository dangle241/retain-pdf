...
/** Initial state compatible with old flat field names */
...
export function createRecentJobsStore(
  ...
  return createStore<RecentJobsState, RecentJobsActions>({
    ...
    actions: {
      ...
      // Soft resetReset pagination cursor only, keep current items. items。
      // Legacy implementation items:[] Will cause silent/Full reload Grid blanks pre-response (homepage flicker root cause).
      // After new data arrives setItems / commitRecentJobsEmpty Atomic replacement.
