...
export async function runOcrTokenValidation({
  ...
  try {
    ...
    if (showResult) {
      ...
      const message = result.summary || `Kết quả kiểm tra ${definition.label} Token: ${result.status || "unknown"}`;
    ...
    resetOcrValidationRuntime({ state, credentialsStatePort, legacyRuntimePort });
    if (showResult) {
      setOcrValidationMessage(`Kiểm tra ${definition.label} Token thất bại, vui lòng thử lại sau.`, "error", definition.id);
    }
    return {
      ok: false,
      status: "network_error",
      summary: `Kiểm tra ${definition.label} Token thất bại, vui lòng thử lại sau.`,
    };
  }
}
...
export async function runDeepSeekConnectivityCheck({
  ...
  if (showResult) {
    setDeepSeekValidationMessage("Đang kiểm tra API mô hình…");
  }
  try {
