...
export function mountUploadFeature({
  ...
  function validatePageRanges() {
    ...
    if (maxPage && start && end && Number(end) - Number(start) + 1 > maxPage) {
      setText("error-box", `Phạm vi không được vượt quá ${maxPage} trang`);
      return false;
    }
  ...
  async function handleFileSelected() {
    ...
    if (file.size > frontMaxBytes) {
      setText("error-box", `Frontend hiện giới hạn PDF ở mức ${formatByteLimit(frontMaxBytes)} trở xuống`);
    ...
    if (frontMaxPageCount && countPdfPages) {
      ...
      try {
        ...
        if (!Number.isFinite(localPageCount) || localPageCount <= 0) {
          setText("error-box", "Không thể phân tích PDF; vui lòng kiểm tra tệp có bị hỏng hoặc không thể truy cập hay không.");
    ...
    try {
      ...
      if (refreshDeepSeekBalance) {
        viewPort.showUploadStatus("Tải lên hoàn tất, đang kiểm tra API mô hình…");
        void withTimeout(
          ...
          .then((result) => {
            ...
            if (status === "network_error" || status === "missing_key") {
              viewPort.showUploadStatus("Tải lên hoàn tất; chưa thể xác nhận API mô hình.");
              return;
            }
