// Hướng dẫn phím tắt: biểu tượng bàn phím ở thanh dưới + danh sách nổi; cũng có thể mở bằng h / ?.
...
const SHORTCUT_GROUPS: { title: string; items: { keys: string; desc: string }[] }[] = [
  {
    title: "Chuyển trang",
    items: [
      { keys: "J · ↓ · PgDn", desc: "Trang tiếp theo" },
      { keys: "K · ↑ · PgUp", desc: "Trang trước" },
      { keys: "Home / End", desc: "Trang đầu / Trang cuối" },
      { keys: "Bấm số trang ở thanh dưới", desc: "Nhập số trang để chuyển" },
    ],
  },
  {
    title: "Thu phóng",
    items: [
      ...
      { keys: "0", desc: "Đặt lại theo mặc định của chế độ" },
      { keys: "Bấm phần trăm", desc: "Đặt lại theo mặc định của chế độ" },
    ],
  },
  {
    title: "Chế độ",
    items: [
      { keys: "1", desc: "Nguyên văn" },
    ...
    const onKey = (event: KeyboardEvent) => {
      ...
      if (key === "?" || key === "h" || key === "H" || key === "/") {
        ...
        if (key === "/" && !event.shiftKey) {
          // Không dùng riêng / làm trợ giúp để tránh bấm nhầm.
          return;
        }
        event.preventDefault();
        setOpen((v) => !v);
    ...
    <div className="reader-react-shortcuts" ref={rootRef} data-reader-shortcuts="">
      ...
      {open ? (
        ...
        >
          ...
          <p className="reader-react-shortcuts-foot">Phím tắt không hoạt động khi con trỏ ở ô nhập</p>
        </div>
      ) : null}
    </div>
  );
