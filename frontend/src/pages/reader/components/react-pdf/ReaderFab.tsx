// Nút công cụ nổi có thể kéo (FAB): bấm để mở menu, kéo để đổi vị trí.
// Menu: Chú thích + Tải xuống (bản gốc / bản dịch / đối chiếu).
...
export type ReaderFabProps = {
  /** ID công cụ đang mở; null nghĩa là đóng tất cả. */
        ...
        >
          ...
          {READER_TOOLS.map((tool, index) => {
            ...
            if (tool.id === "notes" && !isActive && notesCount > 0) {
              sub = `${notesCount} chú thích`;
            }
            if (disabled) {
              sub = "Cần mở trình đọc tác vụ";
            }
            return (
              <button
                key={tool.id}
          ...
          <div className="reader-fab-section" role="group" aria-label="Tải xuống">
              ...
              {downloadItems.map((action, index) => {
                ...
                return (
                  <button
                    ...
                    disabled={!enabled}
                    title={enabled ? `Tải ${meta.label}` : reason}
            ...
            {downloadItems.every((a) => !trimReaderDownloadString(urls[a])) ? (
              <p className="reader-fab-empty">Đầu ra chưa sẵn sàng</p>
            ) : null}
          </div>
        </div>
      ) : null}
