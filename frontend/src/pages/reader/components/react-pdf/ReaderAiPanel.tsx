// Cửa sổ nổi hỏi đáp AI: luồng assistant-ui + cửa sổ hội thoại + mở cửa sổ từ nhánh.
...
export type ReaderAiPanelProps = {
  ...
  sourceOnly: boolean;
  onClose: () => void;
  /** page_idx bắt đầu từ 0; trình đọc gọi goToPage(page_idx+1). */
...
export function ReaderAiPanel({
  ...
  const handleBranch = useCallback(async (assistantId: string) => {
    ...
    if (ok) {
      setBranchNotice(
        "Đã lưu cuộc trò chuyện mới (fork-n-tên-gốc): nội dung đến câu trả lời này đã được sao chép, cuộc trò chuyện gốc không đổi. Có thể chuyển ở danh sách phía trên.",
      );
      window.setTimeout(() => setBranchNotice(""), 6000);
    }
  }, [branchFromAnswer]);

  // Không nhảy PDF trong thời gian khóa phân nhánh/đổi hội thoại để tránh bấm nhầm trích dẫn.
    ...
    >
      {sourceOnly || !jobId ? (
        <div className="reader-float-ai-empty">
          ...
          <span>Vui lòng mở trình đọc từ tác vụ rồi thử lại</span>
        </div>
      ) : (
