// Bản xem trước Markdown nổi: sản phẩm Markdown nhận dạng/bản dịch của tác vụ.
...
export function ReaderMarkdownPanel({
  ...
  useEffect(() => {
    ...
    async function load() {
      if (sourceOnly || !jobId) {
        setStatus("Chế độ đọc tài liệu gốc không cung cấp đầu ra Markdown");
      ...
      try {
        const rawPayload = await defaultReaderDataPort.loadMarkdownPayload(jobId);
        ...
        template.content.querySelectorAll("img[src]").forEach((img) => {
          // Phân giải images/... tương đối thành địa chỉ API tuyệt đối bằng base để tránh 404 khi gắn dưới cùng origin của reader.html.
        ...
        // Ảnh được bảo vệ → tải blob kèm X-API-Key (<img> không thể gửi header xác thực).
        ...
        await Promise.allSettled(images.map(async (img) => {
          ...
          try {
            ...
            fallback.textContent = `[Ảnh tạm thời không khả dụng]`;
            fallback.title = src;
            img.replaceWith(fallback);
          }
        }));
        if (!cancelled && failed > 0 && failed === images.length && images.length > 0) {
          setStatus(`Tải ${failed} ảnh thất bại. Hãy xác nhận API có thể truy cập và X-API-Key đã được cấu hình.`);
        }
      } catch (err) {
        if (cancelled) return;
  ...
  return (
    <ReaderFloatShell
      ...
      open={open}
      title="Markdown"
      subtitle="Đầu ra nhận dạng và dịch · Kéo để di chuyển"
