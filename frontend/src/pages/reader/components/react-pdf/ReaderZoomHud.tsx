// Thanh dưới: số trang (bấm để chuyển) + thu phóng +/- / đặt lại mặc định theo chế độ.
...
export type ReaderZoomHudProps = {
  ...
  /** Khi bấm phần trăm, đặt lại mức thu phóng mặc định của chế độ. */
...
export function ReaderZoomHud({
  ...
  // zoom chính là "tỷ lệ so với toàn bộ chiều rộng vùng đọc": 0.5 → 50%, 1 → 100%.
  ...
  const resetLabel = "50% (nửa màn hình, lấp đầy khi đối chiếu)";
  ...
  return (
    <div className="reader-react-hud" data-reader-hud="true">
      <div className="reader-react-hud-group" aria-label="Số trang">
        {editing ? (
          ...
          <button
            ...
            aria-label={numPages > 0 ? `Chuyển trang; hiện ở trang ${currentPage}, tổng cộng ${numPages} trang` : "Số trang"}
            title={numPages > 0 ? "Bấm để nhập số trang cần chuyển" : undefined}
      ...
      <div className="reader-react-hud-group" aria-label="Thu phóng">
        <button
          type="button"
        ...
        <button
          ...
          aria-label={`Đặt lại thành ${resetLabel}`}
          title={resetLabel}
