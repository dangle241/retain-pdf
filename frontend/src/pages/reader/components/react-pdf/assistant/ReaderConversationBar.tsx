// CRUD nhiều hội thoại: chuyển danh sách / tạo mới / đổi tên / xóa.
...
export function ReaderConversationBar({
  ...
  const listId = useId();
  // Định dạng lưu tiêu đề fork thân thiện với máy là "fork-N-tiêu đề gốc" (việc phân tích số thứ tự phụ thuộc vào đó, không đổi dữ liệu lưu),
  // tầng hiển thị chuyển thành "Tiêu đề gốc · Nhánh N"; ô đổi tên vẫn hiển thị giá trị gốc.
  ...
  const handleDelete = (s: ReaderAskSessionSummary) => {
    ...
    const name = s.title || "Cuộc trò chuyện chưa đặt tên";
    const ok = globalThis.confirm?.(`Xác nhận xóa cuộc trò chuyện “${name}”? Thao tác này không thể hoàn tác.`);
    if (!ok) return;
    pickingRef.current = true;
    ...
    >
      <div className="aui-session-row">
        <button
          ...
          aria-label="Chuyển cửa sổ trò chuyện"
      ...
      {open && hasSessions ? (
        <ul
          ...
          role="listbox"
          aria-label="Cuộc trò chuyện trước"
        >
          {sessions.map((s) => {
                ...
                {editing ? (
                  <div className="aui-session-edit">
                    <input
                      ...
                      value={editTitle}
                      maxLength={80}
                      aria-label="Tiêu đề cuộc trò chuyện"
                      disabled={locked}
