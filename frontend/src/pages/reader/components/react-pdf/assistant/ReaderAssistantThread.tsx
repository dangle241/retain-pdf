// Hỏi đáp trình đọc assistant-ui: tách tiến độ khỏi nội dung; khi hoàn tất mới đưa Markdown + công thức + trích dẫn một lần.
...
/** Gợi ý kiểu thanh bên Notion: biểu tượng + tiêu đề ngắn. */
const SUGGESTIONS: Array<{
  ...
  {
    prompt: "Tóm tắt nội dung cốt lõi của tài liệu này bằng vài câu.",
    label: "Tóm tắt trang này",
    icon: BookOpen,
  },
  {
    prompt: "Kết luận chính của tài liệu này là gì?",
    label: "Rút ra kết luận chính",
    icon: ListTree,
  },
  {
  ...
  },
  {
    ...
    label: "Đánh dấu kết quả quan trọng",
    icon: Sparkles,
  },
];
...
export type ReaderAssistantThreadProps = {
  ...
  /** Đường tắt nội dung qua store, bảo đảm token streaming render ngay lập tức. */
  contentByMessageId?: Record<string, string>;
  /** ID assistant đang được tạo (đường tắt status để tránh aui không cập nhật running). */
  streamingAssistantId?: string;
  isRunning?: boolean;
  onJumpCitation?: (citation: AiCitationLike) => void;
  /** Mở cửa sổ hội thoại mới từ câu trả lời của trợ lý (sao chép lịch sử tới điểm này). */
...
function messageIsStreaming(
  ...
  // aui đôi khi không ánh xạ status của ExternalStore sang useMessage; dùng ID đường tắt làm dự phòng.
...
function useViewportStickBottom(
  ...
  useEffect(() => {
    ...
    const scrollBottom = () => {
      // Không buộc cuộn xuống đáy khi remount do phân nhánh/đổi hội thoại, nếu không sẽ có cảm giác "làm mới rồi nhảy trang".
...
function MarkdownText() {
  ...
  // Ưu tiên metadata.custom (ổn định); fallback về map của store.
  const citations = meta.citations.length
    ? meta.citations
    : (citationsByMessageId[meta.storeId] || citationsByMessageId[meta.messageId] || []);
  const rootRef = useRef<HTMLDivElement | null>(null);
  // Streaming: ưu tiên contentByMessageId (đi thẳng qua store/đường tắt), tránh cache aui Parts khiến "trả lời xong mới render".
  const storeText =
    contentByMessageId[meta.storeId]
    || contentByMessageId[meta.messageId]
    || "";
  // Trong lúc streaming không trim phần cuối để tránh mất nửa câu; chỉ trim sau khi hoàn tất.
  const bodyText = streaming
    ? `${storeText || text || ""}`
    : `${storeText || text || ""}`.trim();
  // Khi remount nhánh, ID đổi nhưng nội dung giống nhau: dùng cache để tránh chớp pending.
  ...
  // Streaming: đồng bộ HTML nhẹ, tuyệt đối không dùng MathJax bất đồng bộ (nếu không sẽ "viết xong mới hiển thị").
  ...
  useEffect(() => {
    ...
    // Thu hồi blob URL của lần hydrate trước trước khi ghi đè (render lại tạo blob mới liên tục,
    // blob cũ sẽ rò rỉ tới khi đóng trang; kiểm tra P1-5).
  ...
  // Khi unmount (đổi tin nhắn/đóng cửa sổ nổi), thu hồi blob của bong bóng này.
  useEffect(() => () => {
...
/**
 * Bộ chuyển nút anh em.
 * - path: nhiều nhánh hỏi tiếp dưới cùng một câu trả lời.
 * - answer: nhiều phiên bản câu trả lời thử lại cho cùng một câu hỏi.
 */
function MessageBranchPicker({ kind }: { kind: "path" | "answer" }) {
  ...
  return (
    <BranchPickerPrimitive.Root
      ...
      title={kind === "path" ? "Chuyển giữa các nhánh bắt đầu từ câu trả lời này" : "Chuyển giữa các phiên bản câu trả lời"}
    >
      ...
      <BranchPickerPrimitive.Next asChild>
        <button
          type="button"
          className="aui-branch-btn"
          aria-label={kind === "path" ? "Nhánh tiếp theo" : "Câu trả lời tiếp theo"}
        >
...
/**
 * Mở "cửa sổ hội thoại mới" từ câu trả lời của trợ lý:
 * Sao chép lịch sử root → câu trả lời này sang conversation mới, giữ nguyên hội thoại gốc (tương tự ChatGPT Branch in new chat).
 */
function AssistantMessage() {
  ...
  const handleBranch = async (event: ReactMouseEvent | ReactPointerEvent) => {
    ...
    setForking(true);
    try {
      // Chờ click kết thúc hoàn toàn rồi mới bật lớp che toàn màn hình; tuyệt đối không che nút ngay ở pointerdown.
      ...
      if (!ok) {
        // Khi thất bại cũng dành chút thời gian phản hồi (sessionError nằm trên thanh hội thoại).
  ...
  return (
    <MessagePrimitive.Root className="aui-msg aui-msg-assistant">
      ...
      {!streaming ? (
        ...
        >
          {/* Chuyển giữa nhiều phiên bản câu trả lời cho cùng câu hỏi (vẫn trong cửa sổ hiện tại; dùng "Mở hội thoại mới" bên dưới để thực sự mở cửa sổ mới). */}
          ...
          <button
            ...
            aria-label="Bắt đầu cuộc trò chuyện mới từ câu trả lời này"
            title="Sao chép nội dung đến câu trả lời này và mở cuộc trò chuyện mới để tiếp tục hỏi (cuộc trò chuyện gốc không đổi, tránh lẫn ngữ cảnh)"
          >
            {forking ? (
            ...
            {forking ? "Đang tạo nhánh…" : "Mở cuộc trò chuyện mới"}
          </button>
          <ActionBarPrimitive.Root className="aui-action-bar" hideWhenRunning>
            <ActionBarPrimitive.Reload asChild>
              <button
                ...
                title="Tạo một phiên bản trả lời khác cho cùng câu hỏi (vẫn trong cửa sổ hiện tại)"
                onPointerDown={(e) => e.stopPropagation()}
                onClick={(e) => e.stopPropagation()}
              >
...
function EmptyState() {
  ...
  return (
    <div className="aui-empty">
      ...
      <h2 className="aui-empty-title">Tôi luôn sẵn sàng. Tôi có thể giúp gì cho bạn?</h2>
      <p className="aui-empty-sub">Tìm kiếm và trả lời dựa trên toàn bộ tài liệu hiện tại · Bấm trích dẫn để chuyển trang</p>
...
export function ReaderAssistantThread({
  ...
  // Khi không có khóa mô hình, cấm nhập/gửi (giống trang chủ: chỉ dùng Cài đặt → Thông tin xác thực).
    ...
    <AskUiContext.Provider value={ctx}>
      <ThreadPrimitive.Root className={`aui-thread${missingLlmKey ? " is-llm-locked" : ""}`}>
        <ThreadPrimitive.Viewport
          ...
          turnAnchor="top"
          // Tắt autoScroll của thư viện khi phân nhánh/đổi hội thoại để tránh cả cột nhảy xuống đáy như "làm mới".
        ...
        >
          ...
          <ThreadPrimitive.ScrollToBottom className="aui-scroll-bottom" asChild>
            <button type="button" className="aui-scroll-bottom-btn" aria-label="Cuộn đến mới nhất">
        ...
        {missingLlmKey ? (
          <div className="aui-composer aui-composer-locked" role="alert">
            ...
            <p className="aui-hint">Vào “Cài đặt → Cài đặt API” ở trang chính và nhập Key của mô hình để đặt câu hỏi</p>
          </div>
        ) : (
          <ComposerPrimitive.Root className="aui-composer" compact>
            {/* Thẻ nhập bo góc liền khối kiểu Notion: nội dung + công cụ thanh dưới. */}
            <div className="aui-composer-shell">
              <ComposerPrimitive.Input
                className="aui-input"
                rows={2}
                placeholder="Yêu cầu AI hỗ trợ…"
                submitOnEnter
              />
              <div className="aui-composer-toolbar">
                <span className="aui-composer-chip" title="Phạm vi tìm kiếm">
                  <BookOpen size={12} strokeWidth={2.2} aria-hidden />
                  Tài liệu hiện tại
                </span>
                {/* Dừng/Gửi loại trừ nhau và dùng cùng vị trí: nếu hai nút luôn hiện thì "Dừng" sẽ vô dụng 95% thời gian. */}
            ...
            <p className="aui-hint">Enter để gửi · Shift+Enter để xuống dòng · Bấm [n] trong câu trả lời để chuyển trang</p>
