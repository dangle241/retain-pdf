// Tải URL PDF cần xác thực / mock:// thành { data: Uint8Array } mà react-pdf dùng được.
// Tầng phiên tải xong toàn file rồi mới hiển thị; hook này cũng có thể dùng độc lập.
...
export async function loadProtectedPdfFile(
  ...
  if (!response.ok) {
    const err = new Error(`Không thể tải PDF (${response.status})`) as Error & { status?: number };
...
/** Tải song song nhiều PDF, chỉ resolve khi tất cả hoàn tất; onItem dùng cho nội dung tiến độ. */
...
export function useProtectedPdfFile(
  url = "",
  /** Khi phiên đã tải trước, inject trực tiếp và bỏ qua yêu cầu thứ hai. */
