// Shared scroll shell positioning: restore reading position by "page + in-page ratio".
// Use pre-switch lock state on mode toggle. progressReject during restore. measure(floats to last page).
...
/** Viewport contains element. Optimize rendering.「reading line」Offset from top of scroll container (vs measure/apply / HUD Current page must match */
...
export function pickPageAtFocus(
  ...
  for (const el of pages) {
    ...
    // Hidden column / Unlaid out: skip, avoid measurement 0 High value causes false last-page detection.
  ...
  if (!best) {
    // All below read line → Page 1; all above → Last page
...
export function applyPageScrollProgress(
  ...
  // Page not laid out: failed, wait next round.
...
/** @deprecated Backward compatibility for legacy name. */
...
/**
 * Use locked. progress Restore position.
 * Retry on unstable layout; same progress Repeat apply Idempotent (won't drift further).
 */
export function alignShellToProgress(
  ...
  // Equal column widths/Anchor same point a few more times after page height settles.
...
export function clampPageNumber(page: number, numPages: number): number {
  ...
  // Do not clamp when total pages unknown. 1（AI Reference redirects will all fail as a result. 1 Page)
