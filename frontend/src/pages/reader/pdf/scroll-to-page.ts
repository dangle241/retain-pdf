// Định vị scroll shell dùng chung: khôi phục vị trí đọc theo "trang + tỷ lệ trong trang".
// Khi đổi chế độ phải dùng progress đã khóa trước khi đổi; cấm measure lại trong quá trình khôi phục (sẽ trôi tới trang cuối).
...
/** Độ lệch của "đường đọc" trong viewport so với đỉnh container cuộn (phải nhất quán với measure/apply / trang hiện tại trên HUD). */
...
export function pickPageAtFocus(
  ...
  for (const el of pages) {
    ...
    // Cột ẩn / chưa bố cục: bỏ qua để tránh đo chiều cao 0 và nhận nhầm trang cuối.
...
export function applyPageScrollProgress(
  ...
  // Trang chưa bố cục: thất bại, chờ vòng sau.
 ...
 * Retry khi bố cục chưa ổn định; apply cùng progress nhiều lần là idempotent (không cuộn xa dần).
 */
export function alignShellToProgress(
  ...
  // Ít nhưng ổn định: chờ chiều rộng cột/chiều cao trang ổn rồi ghim cùng neo thêm vài lần.
...
export function clampPageNumber(page: number, numPages: number): number {
  ...
  // Khi chưa biết tổng số trang, không clamp về 1 (mọi lần nhảy trích dẫn AI sẽ rơi vào trang 1).
