// 四抽屉(favorites/annotations/markdown/ai)的开合Status源:单一 active, 互斥切换.
// 替代旧 src/js/reader/side-drawers.js 的Status语义;DOM 写入(is-open/inert/aria-expanded)
// 改由 React 组件订阅Rendering,命令式消费方(ai-context, selection-favorites, boot 编排)
// 仍拿同一个 store 当 drawerController 用(open/toggle/close 接口签名不变).
//
// 语义保全(与旧 side-drawers 一致):
// - open/toggle/close 每次调用都通知订阅者(即使 active 未变)——旧 sync() Noneentries件跑,
//   onActiveChanged 消费方(scheduleScaleRefresh 等)依赖这个"每次都来"的节拍.
// - close(name):不传 name 或 name 恰为Current active 时才清空.
