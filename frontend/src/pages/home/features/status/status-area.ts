...
// Status区(#status-section)可见性 feature.
//
// 3a 只落"可见性 + Events契约"(镜像 ui/status-area-view.js 的 setStatusAreaVisible
// 与 ui/presentation-view.js 的 setWorkflowSectionsView):StatusCard books体yes 3b
// (recent-jobs + job-runtime 蓝图 features/status/)的Scope,这里的 store 届时
// 直接被 StatusCard.jsx 家族复用.
//
// Events契约:每次 setVisible 都 dispatch statusAreaVisibilityChanged(旧世界
// 同款,translation-workflow-dialog 靠它sync upload/status 模式).
...
export function createStatusAreaFeature({
  ...
  // 旧世界从Status卡元素冒泡 returnHome;新世界直接发到 document
  // (消费方 jobRuntimeFeature.returnToHome yes document 级监听,3b 接线)
  function returnHome() {
    if (documentRef?.dispatchEvent && typeof globalThis.CustomEvent === "function") {
  ...
  // setWorkflowSections(job):idle 复位链与 3b runtime-reset total用的回调
