// Thanh nhập AI trang chính: @ chọn tài liệu/bộ sưu tập + chip + gửi.
...
export type HomeAskComposerProps = {
  disabled?: boolean;
  isRunning?: boolean;
  /** Tắt gửi và hiển thị gợi ý khi chưa cấu hình Key mô hình. */
  missingLlmKey?: boolean;
  scopes: HomeAskScope[];
  onScopesChange: (next: HomeAskScope[]) => void;
  onSend: (question: string) => void;
  onStop?: () => void;
  /** hero: ô nhập lớn căn giữa khi trống; dock: thanh dưới hội thoại. */
  variant?: "hero" | "dock";
};
...
export function HomeAskComposer({
  ...
  const scopeHint = (() => {
    if (missingLlmKey) return "Vui lòng nhập API Key của mô hình trong Cài đặt → Cài đặt API trước";
    if (!scopes.length) return "Toàn thư viện · @ tài liệu hoặc bộ sưu tập";
    ...
    <div className={`home-ask-composer home-ask-composer-${variant}${missingLlmKey ? " is-locked" : ""}`}>
      {missingLlmKey ? (
        <div className="home-ask-key-banner" role="alert">
          <p>Chưa cấu hình API Key của mô hình nên không thể nhập hoặc đặt câu hỏi.</p>
          <button
            type="button"
            className="home-ask-key-banner-btn"
            onClick={() => {
              // Nhất quán với cổng tải lên: mở "Cài đặt → Cài đặt API" là entry thông thường duy nhất.
      ...
      {scopes.length > 0 ? (
        <div className="home-ask-chips" aria-label="Phạm vi câu hỏi">
          {scopes.map((s) => (
            <span
              key={scopeKey(s)}
      ...
      <div className="home-ask-composer-shell" aria-disabled={missingLlmKey || undefined}>
        {/* Lớp khóa: chặn mọi đầu vào, chắc chắn hơn chỉ dùng disabled. */}
        {missingLlmKey ? (
          <div
        ...
        <textarea
          ...
          placeholder={
            missingLlmKey
              ? "Vui lòng cấu hình API Key của mô hình trước…"
              : scopes.length
                ? "Tiếp tục hỏi… dùng @ để chọn thêm tài liệu hoặc bộ sưu tập"
                : variant === "hero"
                  ? "Yêu cầu AI hỗ trợ… nhập @ để chọn tài liệu hoặc bộ sưu tập"
                  : "Tiếp tục hỏi… nhập @ để chọn tài liệu hoặc bộ sưu tập"
          }
          onChange={(e) => {
        ...
        {pickerOpen ? (
          <div className="home-ask-picker" role="listbox" id={listId} aria-label="Chọn tài liệu hoặc bộ sưu tập">
            {loadingOpts && !optionsLoaded ? (
              ...
              <div className="home-ask-picker-empty">
                {optionsLoaded ? "Không có tài liệu hoặc bộ sưu tập phù hợp" : "Chưa có dữ liệu"}
              </div>
            ) : (
            ...
            {scopes.length >= MAX_SCOPES ? (
              <div className="home-ask-picker-hint">Chọn tối đa {MAX_SCOPES} phạm vi</div>
            ) : (
              <div className="home-ask-picker-hint">Các tài liệu trong bộ sưu tập sẽ được mở rộng để tìm kiếm</div>
            )}
          </div>
        ) : null}
