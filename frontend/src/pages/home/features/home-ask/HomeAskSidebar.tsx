// Lịch sử bên trái kiểu Notion: thu gọn + đổi tên + nhóm theo thời gian.
...
function groupLabel(updatedAt: string, now = Date.now()): string {
  ...
  if (!Number.isFinite(t)) return "Cũ hơn";
...
function displayTitle(raw: string): string {
  ...
  if (!m) return raw || "Cuộc trò chuyện chưa đặt tên";
  const rest = m[2].trim();
...
export function HomeAskSidebar({
  ...
  const startRename = (s: HomeAskSession) => {
    if (busy) return;
    setEditingId(s.id);
    // Ô sửa hiển thị tên gốc đã lưu; kể cả có tiền tố fork-n- vẫn cho sửa toàn bộ.
  ...
  if (collapsed) {
    return (
      <aside
        ...
        aria-label="Lịch sử trò chuyện (đã thu gọn)"
      >
        <button
          type="button"
  ...
  return (
    <aside className="home-ask-sidebar" aria-label="Lịch sử trò chuyện">
      ...
      <div className="home-ask-sidebar-scroll">
        {loading && sessions.length === 0 ? (
          ...
          <p className="home-ask-sidebar-empty">Chưa có lịch sử trò chuyện</p>
        ) : (
          groups.map((g) => (
                      ...
                      {editing ? (
                        ...
                        <>
                          <button
                            ...
                            disabled={busy}
                            title={`${title} (bấm đúp để đổi tên)`}
                          ...
                          <button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              const ok = globalThis.confirm?.(`Xóa cuộc trò chuyện “${title}”?`);
                              if (ok) onDelete(s.id);
                            }}
                          >
