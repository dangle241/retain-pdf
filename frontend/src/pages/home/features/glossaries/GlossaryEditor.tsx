// Glossary Editor Table (compare with glossary-manager-dialog-template.js's
// .glossary-editor-panel Table block + view.js:appendGlossaryEntryRow Mirror by column)。
//
// .glossary-editor-panel Table block + view.js:appendGlossaryEntryRow Mirror by column).
// Imperative DOM Row actions â Structured array + .map rendering (Blueprint Â§3): entries All from
// glossaries-store.js draft.entries, Each cell is controlled input/select, onChange directly
...
// eslint-disable-next-line react/no-array-index-key -- Row unstable id (Old world also purely positional DOM row), Index key equivalent to legacy behavior
      ...
      <div id={GLOSSARY_DOM_IDS.entriesEmpty} className={hasEntries ? "hidden" : undefined}>
        {!hasEntries ? (
          <EmptyState
            instrument="spectrum"
            title="No entries."
            hint="Add source and translation. Your terms take priority during translation."
          />
        ) : null}
      </div>
    </div>
