// Gỡ lỗi bản dịch: danh sách item + phân trang; bản viết lại JSX của
...
// + chuyển ba trạng thái DOM) thành cấu trúc; assert từng mục thay cho assert markup (thiết kế §1.1).
// Các hàm định dạng thuần (finalStatusOf/finalStatusLabel/finalStatusClass/previewText/
// pageNumberOf/errorTypesOf) được giữ lại và import trực tiếp.
...
export function TranslationItemsPanel({ translation, onSelect, onChangePage }) {
  ...
  const meta = loading
    ? "Đang tải..."
    : `Tổng cộng ${total} mục, trang này ${list.length} mục, offset ${offset}, limit ${limit}`;
  ...
  return (
    <section className="translation-debug-column translation-debug-column-list">
      <div className="translation-debug-subhead"><h4>Danh sách mục</h4><span id={ids.itemsMeta} className="status-panel-note">{meta}</span></div>
      <div className="translation-panel-body">
        <div id={ids.itemsLoading} className={loading ? "events-empty" : "events-empty hidden"}>Đang tải mục dịch...</div>
        <div id={ids.itemsEmpty} className={!loading && !hasItems ? "events-empty" : "events-empty hidden"}>
          {translation.itemsErrorText || "Không có mục dịch phù hợp"}
        </div>
