// Gỡ lỗi bản dịch: chi tiết item + replay; bản viết lại JSX
// features/status-detail/translation-renderer.js#renderTranslationItemDetail/
// có cấu trúc của #renderTranslationReplay (nối markup). Các hàm định dạng thuần
// (boolLabel/diagnosticsOf/normalizeRoutePath/routePathOf/pageNumberOf/
// finalStatusOf/fallbackToOf/degradationReasonOf) được giữ lại và import trực tiếp;
// renderField/renderTextBlock được thay bằng hai component JSX InfoRow/TextBlock.
...
function ItemDetailBody({ payload }) {
  ...
  return (
    <>
      ...
      <TextBlock label="Nguyên văn" value={item.source_text || ""} />
      <TextBlock label="Bản dịch đã ghi ra đĩa" value={item.translated_text || item.translation_unit_translated_text || item.group_translated_text || ""} />
      <TextBlock label="Bản dịch sau khi bảo vệ" value={item.protected_translated_text || item.translation_unit_protected_translated_text || item.group_protected_translated_text || ""} />
...
export function TranslationItemDetailPanel({ translation, onReplay }) {
  ...
  const emptyText = translation.itemErrorText
    || (translation.selectedItemId ? "Vui lòng chọn một mục bên trái" : "Không có mục nào để xem");
  const meta = loading
    ? "Đang tải..."
    : hasItem
  ...
  const replayStatus = translation.replayLoading
    ? "Đang chạy lại..."
    : hasReplayResult
      ? (replay.payload.replay_error ? "Chạy lại trả về lỗi" : "Chạy lại hoàn tất")
      : (translation.replayErrorText || "-");

  return (
    <section className="translation-debug-column translation-debug-column-detail">
      <div className="translation-debug-subhead"><h4>Chi tiết mục</h4><span id={ids.itemMeta} className="status-panel-note">{meta}</span></div>
      <div className="translation-panel-body translation-panel-body-detail">
        <div id={ids.itemLoading} className={loading ? "events-empty" : "events-empty hidden"}>Đang tải chi tiết mục...</div>
      ...
      <div className="translation-replay-actions">
        <button id={ids.itemReplay} type="button" className="button-link secondary" disabled={!hasItem} onClick={onReplay}>Chạy lại mục hiện tại</button>
