// StatusDetailDialog 的开合Status实例(用 state/dialog-store.js 通用工厂,
// 蓝图 §1"新 store"清单第二items).payload 携带 { activeTab },open(tabName)
// 与 activateTab(tabName) 都直接调用 dialogStore.open({ activeTab }) ——
// createDialogStore().open() 对已 open 的Status只合并 payload, 不重复触发
