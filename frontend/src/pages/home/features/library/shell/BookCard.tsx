// BookCard â Library First Class UI Component: Pure "shell".
//
// Shell handles it.:
//   - Cover / Placeholder / Status badge / Progress bar.
//   - Title + Subtitle
//   - Tap card body → Open details(Or batch select)
//   - hover Render on mask actions Button list
//
// Shell not responsible:
//   - Define buttons and their actions (injected via props.actions)
//   - Translation / Delete / Collections and other business logic (Place in action.onClick or BookDetail)
//
// DefaultButton visible ../actions/ → buildDefaultBookCardActions。
// Add button = caller assembles larger actions array, no changes needed in this file.
...
/** memo / Shared list rowitem Render signature (re-render only on progress/title change). */
...
/**
* Single circular action button on shell (cover hover area).
 * Can be used externally alone. import Reuse.
 */
export function BookCardActionButton({
  ...
  const label = `${action?.label || action?.id || "Actions"}`.trim();
  return (
    <button
      type="button"
...
// Compatibility with old props: actions not explicitly passed are still lower priority than onReader/onReadSource
...
function BookCardImpl({
  ...
  function openTarget() {
    ...
    // Prioritize book details (including in-progress status) Tab); no longer use selectJob Open legacy workflow dialog
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* In progress: center cover loading// No close button in top-right corner OCR/翻译/Render (prone to truncation) */}
        {processing ? <BookCardProcessingOverlay /> : null}

        {/* Top-right final state/Collection: Forbidden truncate/flex Collapse, otherwise「已翻译」Truncated to ellipsis */}
        {badge && !processing ? (
...
// Factory re-exportRead / Translate independent modules, see book-card-actions/
export {
