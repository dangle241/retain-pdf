// home pages组合根: 只做"顺序接线", 不写业务, 不堆 import.
//
// 规则: 
//   1. 所有 ../../../js/* 只在 composition/external.ts
//   2. 各 create* 工厂返回自己的 bag, 这里显式赋值, 禁止 Object.assign(ctx)
//   3. features yes唯一可变注册表；晚绑定通过它Done
//   4. job-runtime / recent-jobs / artifacts 在 composition Stage一次挂齐
...
export function createHomeComposition({
  ...
  // 下层 view/runtime 工厂的默认参 `= {}` 会让 TS 丢掉None默认字段；运行时入参正确, 此处放宽.
  ...
  // bridge required statusDetail holder(后续 createStatusDomain 写入)
  ...
  // recent-jobs 的 scheduleRefresh 会sync读 isWorkflowOpen(DOM data-open)；
