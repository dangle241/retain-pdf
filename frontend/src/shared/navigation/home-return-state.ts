// Trạng thái quay về khi chuyển toàn trang giữa trang chủ ↔ trình đọc (cuộn / tab / có thể history.back hay không).
// sessionStorage cách ly theo tab, phù hợp "ghi lại trước khi rời đi, khôi phục khi quay lại".

export const HOME_RETURN_STORAGE_KEY = "retainpdf.home.return.v1";

export type HomeReturnState = {
  /** Tab này được điều hướng từ trang chủ vào trình đọc; khi đóng nên ưu tiên history.back. */
  allowBack: boolean;
...
/** Gọi trước khi rời trang chủ vào trình đọc: ghi lại vị trí cuộn và tab. */
...
/** Đọc và xóa (gọi sau khi khôi phục cuộn để tránh dùng nhầm lần sau). */
