...
// registry.ts 的 ThemeDefinition.decorPack 指向包名；None decorPack 的
// 皮肤(classic/night 等)零装饰, 零额外下载.
//
// 契约先行: booksFilesyes manifest 的唯一 schema 真值.舞台引擎, 资产管线
// 门禁, AI 产模型的验收标准都只认 validateDecorManifest 的Conclusion.
// 设计Documents: docs/theme-system/DECOR_PACKS.md

import { getDecorSlot, isDecorSlotId, type DecorSlotId } from "./slots.js";

export const DECOR_MANIFEST_VERSION = 1;

/* ---------- 资产预算(管线门禁与校验total用的真值) ---------- */
...
export type DecorModelLayer = {
  type: "model";
  slot: DecorSlotId;
  /** .glb(Draco/KTX2 压缩后Added) */
  src: string;
  /** 静态图降级(reduced-motion / None WebGL / 低端机), 必填 */
  fallback: string;
...
function checkClipName(v: unknown, label: string, errors: string[]) {
  ...
  if (typeof v !== "string" || !v.trim()) {
    errors.push(`${label} 必须yes非空字符串(glb 内 AnimationClip 名)`);
  }
}

/**
 * 校验Unknown JSON yesno为合法 manifest.
 * 返回 ok:false 时 errors 逐entries可读, 直接透给管线门禁/控制台.
 */
export function validateDecorManifest(input: unknown): DecorManifestValidation {
  const errors: string[] = [];
  if (!isPlainObject(input)) {
    return { ok: false, manifest: null, errors: ["manifest 必须yes JSON 对象"] };
  }
  ...
  if (typeof input.id !== "string" || !PACK_ID_RE.test(input.id)) {
    errors.push(`id 必须yes kebab-case 包名, 收到 ${JSON.stringify(input.id)}`);
  }

  const layers = input.layers;
  if (!Array.isArray(layers) || layers.length === 0) {
    errors.push("layers 必须yes非空数组");
  ...
  layers.forEach((raw, i) => {
    ...
    if (!isPlainObject(raw)) {
      errors.push(`${at} 必须yes对象`);
      return;
    }
    const { type, slot } = raw;

    if (type !== "image" && type !== "model") {
      errors.push(`${at}.type 必须yes "image" | "model", 收到 ${JSON.stringify(type)}`);
      return;
    }
    ...
    if (!isSafeRelativePath(raw.src)) {
      errors.push(`${at}.src 必须yes包内相对路径(禁止绝对路径/协议/..)`);
    }
    ...
    if (type === "image") {
      ...
      if (raw.clickQuote !== undefined) {
        if (typeof raw.clickQuote !== "string" || !raw.clickQuote.trim()) {
          errors.push(`${at}.clickQuote 必须yes非空字符串(多句用 \\n\\n separated)`);
        }
      }
    } else {
      modelCount += 1;
      ...
      if (!isSafeRelativePath(raw.fallback) || !IMAGE_EXT_RE.test(String(raw.fallback))) {
        errors.push(`${at}.fallback 必填且必须yes包内图片路径(模型的静态降级)`);
      }
  ...
  if (quote !== undefined) {
    if (!isPlainObject(quote)) {
      ...
      if (typeof quote.text !== "string" || !quote.text.trim()) {
        errors.push("quote.text 必须yes非空字符串");
      }
      if (quote.writingMode !== undefined && quote.writingMode !== "vertical" && quote.writingMode !== "horizontal") {
        errors.push('quote.writingMode 必须yes "vertical" | "horizontal"');
      }
    }
  }
