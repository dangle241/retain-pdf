// 装饰锚点(slot)注册表: 装饰Theme的"布局契约".
//
// 设计原则(docs/theme-system/DECOR_PACKS.md): 
// - Tools UI 永远yes DOM；装饰层只能挂在下列具名锚点上, 不得自造坐标.
// - manifest 声明"资产挂在哪个 slot", slot 在哪, 多大, 什么层级由
//   舞台 CSS(后续 DecorStage)统一实现——资产侧与布局侧解耦.
// - 新增锚点 = 此处登记 + 舞台 CSS 补一entriesLocate；manifest 校验自动放行.
//
// 层级带(z-index band, 具体数值由舞台 CSS 统一m配): 
//   bg  < Tools UI 背板 < mid < Tools UI 内容 ...之外的边缘 < fg
...
export type DecorSlotDefinition = {
  ...
  id: string;
  band: DecorLayerBand;
  /** 大致区域(百m比语义仅作Documents提示, 真值在舞台 CSS) */
  area: string;
  /** true = 允许压到Tools UI 边缘之上(仅 fg 带可为 true) */
  overUi: boolean;
...
export const DECOR_SLOTS: readonly DecorSlotDefinition[] = [
  ...
  // 右下前景位: right-bottom 的 fg 版——required人物/道具压在面板之上的场合
