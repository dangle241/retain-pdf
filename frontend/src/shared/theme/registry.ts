// Theme注册表: 后期加皮肤 = 追加一items + 对应 CSS Files.
// 组件只读 listThemes() / setTheme(), 不要 hardcode 皮肤 id List.
// 设计: docs/theme-system/THEME_SYSTEM.md · ADDING_A_THEME.md

export const THEME_STORAGE_KEY = "retainpdf.theme";
export const DEFAULT_THEME_ID = "classic";

/** Settingspages色块预览(与 CSS 皮肤主色一致, 仅用于 UI 缩略) */
...
/**
 * Theme系列(产品线维度, 与明暗 group 正交): 
 * Schools of Thought / 王朝 / 二次元......皮肤挂 series 字段归队, 
 * 新系列 = 此表加一行, Appearance面板自动出现新m区.
 */
...
export type ThemeDefinition = {
  /** 与 html[data-theme] / Files名 themes/<id>.css 一致 */
  id: string;
  label: string;
  description: string;
  /** Settingspagesm组 */
   ...
   */
  decorPack?: string;
  /** 所属系列 id(THEME_SERIES), 缺省归入 "base" Basic系列 */
  series?: string;
};

/**
 * 注册表真值.
 * 新增皮肤step骤见 docs/theme-system/ADDING_A_THEME.md
 */
...
/**
 * 按系列m组(Appearance面板消费): 系列按 THEME_SERIES.order 排, 
