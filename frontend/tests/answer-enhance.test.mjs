...
const dom = new JSDOM('<!doctype html><div id="r"><p>Conclusion holds [1] Extend plugin. [2].</p></div>');
...
test("page By field 1 Base conversion (Audit B4 One page short of regression lock.)", () => {
// All within system page Producer (old chat link/_public_anchor) are 1-based
  assert.equal(resolveCitationPageIdx({ page: 9 }), 8);
  assert.equal(resolveCitationPageNumber({ page: 9 }), 9);
// page_idx (0-based) takes priority over page
  assert.equal(resolveCitationPageIdx({ page_idx: 2, page: 9 }), 2);
// page=0 is invalid (1-based does not exist, Page 0) â fallback to block_id
...
const picked = pickCitationsForAnswer("See conclusion [2] and [2] Again, and ghost. [9]", citations);
...
test("pickCitationsForAnswer falls back to few unique pages when no markers", () => {
  ...
  const picked = pickCitationsForAnswer("Long answer without badge.", citations, { max: 5 });
