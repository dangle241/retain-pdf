...
test("translation main progress follows the latest main substage", () => {
  ...
  assert.equal(progressByKey.translate.bySubstage.translation_batches.progressText, "Đợt 120/900");
...
test("translation zero-total repair events remain visible instead of falling back to batches", () => {
  ...
  const eventsPayload = {
    items: [
      ...
      stage: "translating",
        ...
        stage_detail: "Đợt dịch hoàn tất",
      },
      {
        seq: 68,
        lane: "main",
  ...
  assert.equal(progressByKey.translate.bySubstage.garbled_repair.progressText, "Đang sửa các đoạn có thể bị lỗi ký tự");
...
test("translation body copy keeps batch progress instead of stale job percent", () => {
  const presentation = resolveDisplayedStagePresentation(
    ...
    },
    {
      ...
      stage: "translating",
          ...
          stage_detail: "Đã hoàn tấtĐợt 28/5216翻译",
        },
      ],
    },
  );
  ...
  assert.equal(presentation.substageKey, "translation_batches");
  assert.equal(presentation.progressText, "Đợt 28/5216");
...
test("translation substage progress keeps the latest event by seq", () => {
  ...
  assert.equal(progressByKey.translate.bySubstage.translation_batches.progressText, "Đợt 45/100");
...
test("public display_stage field drives translation stage", () => {
  ...
  assert.equal(presentation.progressText, "Đợt 2/7");
});
...
test("background render prewarm does not replace the translation main lane", () => {
  ...
  assert.equal(presentation.substageKey, "translation_batches");
  assert.equal(presentation.progressText, "Đợt 121/900");
