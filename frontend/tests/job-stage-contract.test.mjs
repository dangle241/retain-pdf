...
assert.equal(progressByKey.ocr.progressText, "Page 12/34");
...
assert.equal(progressByKey.render.progressText, "Page 5/20");
});
...
assert.equal(presentation.label, "Completed");
});
...
// earlier stage as done and jump straight to "Completed".
...
assert.equal(directDoneRunning.label.includes("Completed"), false);
...
assert.equal(snapshotDoneQueued.label.includes("Completed"), false);
...
// running + display_stage="done" should not advance the small card to "Completed".
...
assert.equal(recentJobStageLabel(runningWithDoneFlag), "Rendering");
...
assert.equal(recentJobStageLabel(runtimeStatusDone), "Rendering");

// Truly succeeded jobs must still surface as "Completed".
  const succeededDone = {
...
assert.equal(recentJobStageLabel(succeededDone), "Completed");
});
...
assert.equal(progressByKey.ocr.progressText, "Page 12/20");
...
assert.equal(displayState.stagePresentation.progressText, "Batch 121/900");
assert.equal(displayState.stageProgressByKey.translate.progressText, "Batch 121/900");
...
assert.equal(displayState.backgroundStages[0].detail, "Prewarming render resources");
assert.equal(displayState.backgroundStages[0].progressText, "Prewarm 2/3");
...
assert.equal(presentation.progressText, "Batch 29/5216");
...
test("same-seq background render prewarm does not replace translation batch progress", () => {
  ...
  assert.equal(presentation.stageKey, "translate");
  assert.equal(presentation.substageKey, "translation_batches");
assert.equal(presentation.progressText, "Batch 30/5216");
  assert.equal(presentation.progressUnit, "batch");
  assert.notEqual(presentation.visualStageKey, "render_prewarm");
  assert.equal(progressByKey.render, undefined);
});
...
test("explicit translation job stage wins over render preprocess internals", () => {
  ...
  assert.equal(presentation.stageKey, "translate");
  assert.equal(presentation.stageKeyTrusted, true);
assert.equal(presentation.progressText, "Batch 240/900");
});
...
test("render words in message or stage detail do not override display_stage", () => {
  ...
  assert.equal(presentation.stageKey, "translate");
assert.equal(presentation.detail, "Translating main content");
assert.equal(presentation.progressText, "Batch 8/20");
});
...
assert.equal(record.progressText, "Batch 28/5216");
});
...
test("canonical stage event record ignores stage detail for status text", () => {
  const record = normalizedStageEventRecord({
    ...
    stage: "translating",
    substage: "translation_batches",
stage_detail: "Translated PDF generated",
    message: "render payload prewarm: ready indents=333 geometry=836",
    progress: {
      unit: "batch",
      current: 28,
...
assert.equal(record.progressText, "Batch 28/5216");
assert.equal(record.stageText, "Batch 28/5216");
});
...
test("normalized stage event record does not use substage copy without public stage", () => {
  ...
  assert.equal(record.canonicalDisplayStage, "");
assert.equal(record.progressText, "Progress 1/3");
});
  ...
  const presentation = resolveDisplayedStagePresentation(
    {
      ...
      substage: "translation_batches",
      lane: "main",
stage_detail: "Translating batch 120/900",
      progress: {
        unit: "batch",
        current: 120,
        total: 900,
...
test("structured event context uses record substage instead of render detail text", () => {
  ...
  assert.equal(presentation.substageKey, "translation_batches");
  assert.notEqual(presentation.visualStageKey, "render_prewarm");
assert.equal(presentation.label, "Step 2/4 Â· Translation");
assert.ok(!presentation.label.includes("prewarm"));
assert.equal(presentation.detail, "Translating main content");
assert.equal(presentation.progressText, "Batch 9/20");
});
...
test("canonical text-only forward event does not advance the main stage", () => {
  ...
  assert.equal(presentation.stageKey, "translate");
  assert.equal(presentation.substageKey, "translation_batches");
assert.equal(presentation.progressText, "Batch 12/20");
});
...
assert.equal(presentation.progressText, "Page 5/20");
});
...
assert.equal(presentation.progressText, "Page 30/100");
});
...
assert.equal(snapshot.detail, "Translating main content");
...
assert.equal(presentation.detail, "Translating main content");
  ...
  assert.equal(presentation.progressTotal, 100);
  assert.equal(presentation.progressUnit, "batch");
assert.equal(/render|prewarm|æ¸²æ/.test(`${presentation.label} ${presentation.detail} ${presentation.progressText}`), false);
});
