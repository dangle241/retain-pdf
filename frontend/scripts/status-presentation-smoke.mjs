  ...
  const events = {
    items: [
      ...
      },
      {
        stage: "translating",
        event_type: "stage_progress",
        stage_detail: "Đang dịch nội dung chính, lô 18/55",
        progress_current: 18,
        progress_total: 55,
      },
    ],
...
function checkTranslatePresentationUsesBatchProgressWhenDetailMentionsOcr() {
  ...
  assertEqual(presentation.progressText, "Lô 18/55", "Translate batch progress with OCR text");
}
...
function checkTranslatePresentationIgnoresOcrEvents() {
  ...
  assertEqual(presentation.progressText, "Lô 18/55", "Translate falls back to job batch progress");
}
...
function checkContinuationReviewUsesPageProgress() {
  const job = {
    ...
    stage_detail: "Bắt đầu kiểm tra đoạn liên tục qua cột/trang",
    progress_current: 4,
    progress_total: 12,
  };
  ...
  assertEqual(presentation.label, "Bước 2/4 · Kiểm tra liên tục", "Continuation review label");
...
function checkPagePoliciesUsePageProgress() {
  const job = {
    ...
    stage_detail: "Bắt đầu thực hiện chiến lược trang và phân loại khối",
    progress_current: 6,
    progress_total: 12,
  };
  const presentation = resolveDisplayedStagePresentation(job, { items: [] });
...
function checkTranslateUsesLatestSubstageProgress() {
    ...
    items: [
      {
        stage: "translating",
        event_type: "stage_transition",
        stage_detail: "Bắt đầu giai đoạn dịch thuật thuần",
      },
      {
        stage: "continuation_review",
        event_type: "stage_transition",
  ...
  assertEqual(presentation.label, "Bước 2/4 · Kiểm tra liên tục", "Translate substage label");
...
function checkOcrPercentProgressDoesNotLookLikePages() {
  ...
  assertEqual(presentation.progressText, "OCR đang xử lý", "OCR zero percent text");
}
...
function checkOcrFallbackProgressUsesStageSteps() {
  ...
  assertEqual(presentation.progressText, "OCR chuẩn bị", "OCR fallback text");
...
function checkOcrRealPageProgressIsDeterminate() {
  const job = {
    ...
    stage: "ocr_processing",
    current_stage: "ocr_processing",
    stage_detail: "Nhiệm vụ con OCR: Paddle đang phân tích tệp",
    progress_current: 9,
    progress_total: 24,
  };
  const presentation = resolveDisplayedStagePresentation(job, { items: [] });
...
function checkOcrResultReadyStaysInOcrStage() {
  const job = {
    ...
    stage_detail: "Kết quả OCR provider đã sẵn sàng, đang tải bundle gốc",
  };
  const presentation = resolveDisplayedStagePresentation(job, { items: [] });
  assertEqual(presentation.stageKey, "ocr", "OCR result ready stage");
  assertEqual(presentation.detail, "Kết quả OCR provider đã sẵn sàng, đang tải bundle gốc", "OCR result ready detail");
}
...
function checkOcrUploadWaitingDoesNotLookQueued() {
  ...
  assertEqual(presentation.label, "Bước 1/4 · OCR", "OCR upload waiting label");
  assertEqual(presentation.detail, "Paddle đã nhận nhiệm vụ, đang chờ xếp hàng", "OCR upload waiting detail");
}
...
function checkTranslationSubstageOrderDoesNotPreferBatchWhenReviewing() {
  ...
  assertEqual(presentation.label, "Bước 2/4 · Kiểm tra liên tục", "Continuation review label wins");
...
function checkCompletedStageHasDoneKeyAndNoProgressTextRequirement() {
  ...
  assertEqual(presentation.label, "Hoàn thành", "Completed label");
}
...
function checkFailedStageUsesFailureSummary() {
  const job = {
    ...
    failure: {
      summary: "Typst render thất bại: trang 9 tràn văn bản",
    },
  };
  const presentation = resolveDisplayedStagePresentation(job, { items: [] });
  assertEqual(presentation.stageKey, "failed", "Failed stage");
  assertEqual(presentation.label, "Thất bại", "Failed label");
  assertEqual(presentation.detail, "Typst render thất bại: trang 9 tràn văn bản", "Failed detail uses failure summary");
}
...
function checkRunningFinishedStageStaysInRenderUntilTerminal() {
  const job = {
    ...
    stage: "finished",
    current_stage: "finished",
    stage_detail: "Nhiệm vụ hoàn tất",
  };
  const presentation = resolveDisplayedStagePresentation(job, { items: [] });
  assertEqual(presentation.stageKey, "render", "Running finished transition stays render");
  assertEqual(presentation.label, "Bước 3/4 · Render", "Running finished transition label");
}
    ...
    const presentation = resolveDisplayedStagePresentation(
      {
        ...
        stage_detail: `${workflow} worker đã khởi động`,
      },
      { items: [] },
    );
...
function checkRenderPrepareDoesNotLookLikeOcr() {
  ...
  assertEqual(presentation.label, "Bước 3/4 · Render", "Render prepare label");
}
...
function checkHistoricalOcrProgressCanBeRecoveredFromEvents() {
    ...
    },
    {
      items: [
        ...
        },
        {
          stage: "translating",
          event_type: "stage_progress",
          stage_detail: "Đang dịch nội dung chính, lô 2/9",
          progress_current: 2,
          progress_total: 9,
        },
      ],
  ...
  assertEqual(progressByKey.translate.progressText, "Lô 2/9", "Historical translate progress text");
}
...
function checkFormalEventContractProgressUnits() {
  ...
  assertEqual(progressByKey.translate.progressText, "Lô 8/42", "Formal translate batch progress");
...
function checkOcrZeroPageProgressIsVisibleIndeterminate() {
  ...
  assertEqual(progressByKey.ocr.progressText, "OCR đang xử lý, 33 trang", "OCR zero page progress text");
      ...
      items: [
        {
          ...
          stage_detail: "Kiểm tra liên tục qua cột/trang",
          event_type: "stage_progress",
          progress_unit: "page",
          progress_current: 4,
          progress_total: 18,
...
function checkTranslateProgressTextFallbackParsesStageDetail() {
  ...
  assertEqual(presentation.progressText, "Lô 1292/5216", "Translate stage detail fallback progress text");
...
function checkRenderZeroPageProgressShowsUsefulText() {
  const presentation = resolveDisplayedStagePresentation(
    {
      status: "running",
      stage: "rendering",
      stage_detail: "Bắt đầu render PDF dịch",
    },
    {
      items: [
        {
  ...
  assertEqual(presentation.progressText, "Render chuẩn bị, 533 trang", "Render zero page progress text");
